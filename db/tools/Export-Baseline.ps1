...
#
# Usage:
#   .\Export-Baseline.ps1 [-CommitMessage "describe change"]
#   .\Export-Baseline.ps1 -DryRun
...
Write-Host "Export-Baseline: connecting to $Database@$Host:$Port" -ForegroundColor Cyan
...
$lines += "-- Apply:    psql -U ccdashboard_user -d rtmviewdb -f db/baseline.sql"
$lines += ""
...
$relPath = "db/baseline.sql"
& git -C $RepoRoot add $relPath

$diff = & git -C $RepoRoot diff --cached --stat $relPath
if (-not $diff) { Write-Host "No changes in baseline.sql — nothing to commit." -ForegroundColor Gray; exit 0 }
