...
<#
  PROBE 234 / who-asked      THREE candidates. The operator restarted the SHELL service too.
  ...
  THE CORRECTION THAT REFRAMES THIS [operator, 2026-09-13]: the second time, the SHELL service was
  restarted as well. And my own table already shows what that implies:
      window 2 (engine+adapter restarted) : ChangedUnionUsersData = 0   <- nobody ASKED
      window 3 (engine+adapter+SHELL)     : count = 35 at 02:40:03      <- asked, and got 35 agents
  Both windows had Add > 0 (36 and 423), so union.Users was populated in BOTH. Then the grid after the
  first restart was empty NOT because the engine had nobody to give, but because nothing asked it. That
  also closes a question I left open at 01:59: opening the screen produced no getUsers at all. The Shell
  registers its grid subscription once, when its connection is established (RTMHub.AddGridConnection),
  and after the engine restarts it does not re-register. Restarting the Shell forces registration.
  So a THIRD candidate joins, and it is now the leading one:
    SHELL  - the population was fine; the CONSUMER was stale. Fix: the Shell must re-subscribe on
             reconnect (or the engine must push on a new connection). Nothing to do with the mapping.
  This probe must separate "nothing was populated" from "populated, but nobody asked", per window.
  ...
  are distinguishable:
    ...
    SHELL      - see the correction above: populated either way, but only the window with a Shell
               restart produced a getUsers / count= pair. DECISIVE TEST: in a window with Add > 0 and
               zero getUsers, the subject is the consumer, not the producer.
...
$outf  = Join-Path $OutDir ('234_' + $stamp + '_who-asked.txt')
...
Say ''
Say '===== 0b  the SHELL service - the consumer. Discovered, not assumed ====='
$shellFound = 0
foreach ($svc in @(Get-WmiObject Win32_Service -ErrorAction SilentlyContinue |
                   Where-Object { $_.Name -match '(?i)shell' -or $_.PathName -match '(?i)shell' })) {
    $shellFound++
    Say ('  {0} : state {1} , path {2}' -f $svc.Name, $svc.State, $svc.PathName)
    $ps = Get-Process -Id $svc.ProcessId -ErrorAction SilentlyContinue
    if ($ps) { Say ('      pid {0} started {1}' -f $ps.Id, $ps.StartTime.ToString('yyyy-MM-dd HH:mm:ss')) }
}
Say ('  shell-looking services found : {0}   0 means I could not identify the consumer and say so' -f $shellFound)
...
for ($i = 0; $i -lt $take.Count; $i++) {
    ...
    Say ('      Add {0} , MISS {1}   <- the PRODUCER side' -f $adds.Count, $misses.Count)
    Say ('      6 ASKED  getUsers {0} , count= {1} , PUSH {2} , GridConnection {3}   <- the CONSUMER side' -f `
         $asks.Count, $counts.Count, $pushes.Count, $gridConn.Count)
    foreach ($c in $counts) {
        $cm = [regex]::Match($c.Text, 'count=(\d+)')
        Say ('          {0}  count = {1}' -f $c.Time.ToString('HH:mm:ss.fff'), $cm.Groups[1].Value)
    }
    foreach ($a2 in @($asks | Select-Object -First 6)) { Say ('          ask  {0}  | {1}' -f $a2.Time.ToString('HH:mm:ss.fff'), $a2.Text.Trim().Substring(0, [Math]::Min(110, $a2.Text.Trim().Length))) }
    if ($adds.Count -gt 0 -and $asks.Count -eq 0) {
        Say '      -> THIS WINDOW: the engine DID populate and NOTHING asked it. The empty grid here is a'
        Say '         CONSUMER failure, not a data failure. This is the SHELL candidate, measured.'
    }
