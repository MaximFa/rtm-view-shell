...
<#
  ...
  CORRECTED AFTER ITS FIRST RUN, and the correction is a defect of mine, not of the machine: the verdict
  branches were ordered UNSET before NO ROWS, so an EMPTY table whose sequence was never used came back as
  "the next insert collides" - false, because the first insert takes nextval and meets nothing. Measured on
  2026-09-12: RTSGrid_Statistic on both instances, and RTSGrid_TemplateCell on 5432, are empty. The order is
  now NO ROWS first, and UNSET is a danger only when rows exist. The branch order is part of the predicate.
  THIS FILE HAS NOT BEEN RUN in its corrected form - the numbers in the 2026-09-12 report came from the
  version whose verdict text was wrong while every measured value in it was right.
    ...
    FOR rec IN SELECT table_name FROM information_schema.tables
        ...
        BEGIN
            ...
            IF max_val IS NULL THEN
                verdict := 'NO ROWS - the table is empty, so the first insert takes nextval and collides with nothing; an unset sequence here is harmless'; n_norows := n_norows + 1;
            ELSIF last_val IS NULL THEN
                verdict := 'UNSET WITH ROWS PRESENT - the next insert collides'; n_bad := n_bad + 1;
