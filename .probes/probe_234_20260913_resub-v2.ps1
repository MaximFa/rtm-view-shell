...
<#
  PROBE 234 / resub-v2   BOX B of two. ONE run closing BOTH analyses - shell (PR234-SHELL-RESUB-01) and backend
...
$outf  = Join-Path $OutDir ('234_' + $stamp + '_resub-v2.txt')
...
Say ''
Say '===== 3b  the marker left by BOX A, and the FLAG-LIVE control ====='
...
if ((Safe-Count $markers) -lt 1) {
    Say '  *** no marker from box A. Either it was not run, or its output went elsewhere. Without the'
    Say '      edit time I cannot bound the flag-live control, and an unbounded search would count an'
    Say '      OLD RECV line as proof. Stopping before the restart.'
    Fin $false
}
$marker = $markers[0]
Say ('  using : {0}   mtime {1}' -f $marker.FullName, $marker.LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss'))
$mk = @{}
foreach ($ln in @(Get-Content -LiteralPath $marker.FullName -Encoding UTF8)) {
    if ($ln -match '^([A-Za-z]+)=(.*)$') { $mk[$matches[1]] = $matches[2] }
}
foreach ($k in @('editTime','config','backup','changed')) { Say ('      {0,-12} {1}' -f $k, $mk[$k]) }
...
if (-not [datetime]::TryParseExact("$($mk['editTime'])", 'yyyy-MM-dd HH:mm:ss.fff',
    ...
    Say '  *** the marker editTime does not parse - stopping rather than guessing a boundary'; Fin $false
}
if ("$($mk['changed'])" -eq '1') {
    $script:FlagConfig = "$($mk['config'])"
    $script:FlagBackup = "$($mk['backup'])"
    Say '  box A changed the flag, so THIS box owns restoring it to false.'
} else {
