...
<#
  PROBE 234 / r1-verify-v3      DISCOVERY FIRST. The previous run failed on MY paths, not on the system.
  ...
  WHY v3 EXISTS - my own defect, named before any number. v2 returned FAIL on three items:
...
$outf  = Join-Path $OutDir ('234_' + $stamp + '_r1-verify-v3.txt')
...
foreach ($svcName in @('RTMTwilio_1','RTMService')) {
    ...
    $svcDir = Split-Path -LiteralPath $exe -Parent
...
foreach ($dc in $declared) { [void]$searchDirs.Add((Split-Path -LiteralPath $dc -Parent)) }
...
$adapterLive = @($live | Where-Object { $adapterDir -and ($_.StartsWith($adapterDir, [StringComparison]::OrdinalIgnoreCase)) -or $_ -match '(?i)twilio' })
...
$engLive = @($live | Where-Object { $engineDir -and $_.StartsWith($engineDir, [StringComparison]::OrdinalIgnoreCase) })
