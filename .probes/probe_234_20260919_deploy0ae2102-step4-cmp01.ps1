...
<#
  PROBE  probe_234_20260919_deploy0ae2102-step4-cmp01.ps1
...
Say "RUN"
Say ("  Compare-ToBaseline.ps1 -DBHost 127.0.0.1 -DBPort 5433 -Database rtmviewdb -User ccdashboard_user -BaselineDir <package DB> -OutDir " + $OutputDir)
$comparatorOutput = & powershell.exe -ExecutionPolicy Bypass -NoProfile -File $comparatorPath -DBHost 127.0.0.1 -DBPort 5433 -Database rtmviewdb -User ccdashboard_user -Password $plainPassword -BaselineDir $baselineDir -OutDir $OutputDir 2>&1
...
$deltaFile = @(Get-ChildItem -LiteralPath $OutputDir -Filter 'baseline_delta_rtmviewdb_*.txt' -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1)
if (-not $deltaFile) { Say "  *** no delta file produced - the comparison did not happen. STOP."; Finish $false }
Say ("  " + $deltaFile.FullName + "   " + $deltaFile.Length + " B")
