...
<#
  PROBE  probe_234_20260919_predeploy-state.ps1
  UNIT   PR234-DEPLOY-LIGHT01/L10N - step 1 of the rollout: the state of the machine BEFORE anything is built.
  WHY    Everything my side computed (9 commits, 4 product files) is a delta FROM 0ae2102. That 0ae2102 is
         on the machine is INHERITED, measured on 2026-09-18 by another line, not by me. If the machine
         carries something else today, the delta is different and the package would be built for a state
         that does not exist. This probe turns the inherited assumption into a measurement.
  WHERE  SERVER 234 ONLY. READ ONLY: Get-Service / file hashes / SELECT. Nothing is started, stopped,
         installed or edited. The only file it writes is its own report under C:\RTMView-Ops\output.
  ROLLBACK  Delete the report printed on the last line. Nothing else is touched.
  COUNTERS  A value of -999 means NOT MEASURED. It is never printed as 0: a zero that means
            "the measurement did not happen" is indistinguishable from a real zero, and both read green.
#>
...
$ReportPath = Join-Path $OutputDir ("234_{0}_predeploy-state.txt" -f $RunStamp)
