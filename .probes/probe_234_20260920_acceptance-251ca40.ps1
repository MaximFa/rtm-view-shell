<#
  PROBE  probe_234_20260920_acceptance-251ca40.ps1
  BASED  ON probe_234_20260920_acceptance-0f969d8.ps1 - this evening's acceptance, same shape.
         (That file's own header named a DIFFERENT date than its filename - a stale self-claim
         carried across a copy. Corrected here, and it is the class the linter gate will catch.)
  UNIT   acceptance of the FOURTH Shell package (251ca40): the filter popup closing on a dashboard
         click, and the screen editor localised - 19 keys into all three dictionaries.
...
$ReportPath = Join-Path $OutputDir ("234_{0}_acceptance-251ca40.txt" -f $RunStamp)
$ReportLines = New-Object System.Collections.ArrayList
$ExpectedCommit = '251ca40'
$ExpectedWebDll = '76C00DBCAA2E7A06F960E724BC8FD9A31FF9C22D8827A55BFFD815937B066115'
# RE-TAKEN for this flight, not inherited. These are the sizes measured INSIDE the package on the
# station at 20:48 and confirmed on this machine right after the install at 21:38. The previous
# flight's 74752/84992/68096 are exactly what this install had to move AWAY from: carried over,
# they would make the check unable to fail.
$ExpectedSatellite = @{ 'he-IL' = 76800 ; 'ru-RU' = 86528 ; 'en-US' = 69632 }
...
Say ("ACCEPTANCE OF 251ca40  " + $RunStartedAt.ToString('yyyy-MM-dd HH:mm:ss') + " (machine local clock)  host=" + $env:COMPUTERNAME)
...
'@
    ...
    $queryErr = Join-Path $OutputDir ("234_{0}_acceptance-251ca40.err.txt" -f $RunStamp)
...
Say "END-OF-RUN MARKER: ACCEPTANCE-251CA40-COMPLETE"
