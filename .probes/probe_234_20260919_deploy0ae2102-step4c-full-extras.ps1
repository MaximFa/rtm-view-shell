...
<#
  PROBE  probe_234_20260919_deploy0ae2102-step4c-full-extras.ps1
...
Copy-Item -LiteralPath $shippedTools -Destination (Join-Path $ScratchDir 'tools') -Recurse -Force
foreach ($sibling in @('schema.sql')) {
  Copy-Item -LiteralPath (Join-Path $StagingDir ('DB\' + $sibling)) -Destination (Join-Path $ScratchDir $sibling) -Force -ErrorAction SilentlyContinue
}
foreach ($siblingDir in @('functions','data','setup')) {
  $sourceDir = Join-Path $StagingDir ('DB\' + $siblingDir)
  if (Test-Path -LiteralPath $sourceDir) { Copy-Item -LiteralPath $sourceDir -Destination (Join-Path $ScratchDir $siblingDir) -Recurse -Force }
}
$workingComparator = Join-Path $ScratchDir 'tools\Compare-ToBaseline.ps1'
