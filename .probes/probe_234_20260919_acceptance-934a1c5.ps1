...
<#
  PROBE  probe_234_20260919_acceptance-934a1c5.ps1
  UNIT   PR234-DEPLOY-LIGHT01/L10N step 5 - acceptance of the install done at 12:58 local.
  WHERE  SERVER 234. READ ONLY: Get-Service / Get-Process / file hashes / SELECT / curl.
         Nothing is started, stopped, installed or edited. The adapter is NOT started by hand -
         its lifecycle belongs to the engine (role lesson 2026-09-14).
  WHAT   ProductVersion of the deployed Shell, the adapter watched over three samples a minute apart,
         liveness as a pair, the database naming itself, the feed as an OBSERVATION and not a gate,
         and the legacy RTM service read once more so that someone else's change is not read as ours.
  WHY the feed is not a gate: an empty minute is a quiet hour, not a failure (norm N-16). The feed is
         printed with its numbers and left to the coordinator to read.
  COUNTERS  -999 means NOT MEASURED and is never printed as 0.
  ROLLBACK  None needed: this run writes only its own report under C:\RTMView-Ops\output.
#>
...
$ReportPath = Join-Path $OutputDir ("234_{0}_acceptance-934a1c5.txt" -f $RunStamp)
$ReportLines = New-Object System.Collections.ArrayList
$ExpectedCommit = '934a1c5'
$ExpectedWebDll = '639D048A7EC65C31555EB1F414FF899A1719AC9DC43DEC932DD02E9F9D75A157'
$ExpectedSatellite = @{ 'he-IL' = 73728 ; 'ru-RU' = 83968 ; 'en-US' = 67584 }
...
Say ("ACCEPTANCE OF 934a1c5  " + $RunStartedAt.ToString('yyyy-MM-dd HH:mm:ss') + " (machine local clock)  host=" + $env:COMPUTERNAME)
...
$livenessOk = ($httpOk -and $negOk -and $pipeServed -and (-not $pipeNeg) -and (@($pipes).Count -gt 0))
Say ("  LIVENESS : " + $livenessOk)
Rule
...
'@
    ...
    $queryErr = Join-Path $OutputDir ("234_{0}_acceptance-934a1c5.err.txt" -f $RunStamp)
...
Say "END-OF-RUN MARKER: ACCEPTANCE-934A1C5-COMPLETE"
