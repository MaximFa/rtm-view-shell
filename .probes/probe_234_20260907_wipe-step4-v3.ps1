...
# Declared as two plain parallel arrays, not as an array of hashtables: the hashtable form
# came out EMPTY at run time on 2026-09-07, P2f checked nothing and printed only its heading,
# and the run was saved only because the older count gate still failed. Simple shapes fail loudly.
$ExtraNames  = @("Backup_20260829_1129", "parallel_20260711_130140")
$ExtraExpect = @(879, 3)
...
if (-not (Test-Path $Manifest)) { Say "  *** STOP: evacuation manifest missing"; $fail++ }
    ...
    $exclude = @($AppPres)
    for ($i = 0; $i -lt $ExtraNames.Count; $i++) { $exclude += (Join-Path $Preserve $ExtraNames[$i]) }
...
Say ("  subtrees this box knows about : {0}   (must be 2 - a silent empty list is how P2f checked nothing on 2026-09-07)" -f $ExtraNames.Count)
if ($ExtraNames.Count -ne 2 -or $ExtraExpect.Count -ne 2) { Say "  *** P2f HAS NOTHING TO CHECK - refusing"; $fail++ }
for ($i = 0; $i -lt $ExtraNames.Count; $i++) {
    $name = $ExtraNames[$i]
    $xdir = Join-Path $Preserve $name
    $xexp = $ExtraExpect[$i]
    if (-not (Test-Path $xdir)) { Say ("  *** MISSING PRESERVED SUBTREE: {0}" -f $name); $fail++; continue }
    $man = Join-Path $xdir "manifest.txt"
    ...
    foreach ($r in $rows) {
        $p3 = $r.Split("|",3)
        $to = Join-Path $xdir $p3[2]
        if (-not (Test-Path $to)) { $miss++; continue }
        if ((Get-FileHash $to -Algorithm SHA256).Hash -ne $p3[0]) { $diff++ }
    }
    $onDisk = @(Get-ChildItem $xdir -Recurse -File -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne 'manifest.txt' }).Count
    Say ("  {0,-30} manifest {1,4} / files {2,4} / expected {3,4} / missing {5} / differing {6}" -f $name, $rows.Count, $onDisk, $xexp, "", $miss, $diff)
    if ($rows.Count -ne $xexp -or $onDisk -ne $xexp -or $miss -ne 0 -or $diff -ne 0) { Say ("  *** {0} FAILED" -f $name); $fail++ }
}
...
$exclAfter = @($AppPres)
for ($i = 0; $i -lt $ExtraNames.Count; $i++) { $exclAfter += (Join-Path $Preserve $ExtraNames[$i]) }
...
for ($i = 0; $i -lt $ExtraNames.Count; $i++) {
    $xdir = Join-Path $Preserve $ExtraNames[$i]
    Say ("  preserved {0,-28}: {1}   (must be {2})" -f $ExtraNames[$i], @(Get-ChildItem $xdir -Recurse -File -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne 'manifest.txt' }).Count, $ExtraExpect[$i])
}
