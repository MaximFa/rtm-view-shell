...
# ============================================================================
#  PROBE DEV / parse-check preserve-applysvc   -   READ ONLY, local machine.
...
$target = "D:\Claude\Projects\RTM View Shell\.probes\probe_234_20260906_preserve-applysvc.ps1"
...
Write-Host ("  bytes   : {0}   (expect 10773)" -f $b.Length)
Write-Host ("  BOM     : {0}   (must be True)" -f $bom)
Write-Host ("  NUL     : {0}   (must be 0)" -f $nul)
Write-Host ("  sha256  : {0}" -f $sha)
Write-Host  "  expected: CE7EA8D2B994CCD0FE4E0ADFC5229F72F1B42E8218917D70133C24BFF49D65B9"
$hashOk = ($sha -eq "CE7EA8D2B994CCD0FE4E0ADFC5229F72F1B42E8218917D70133C24BFF49D65B9")
