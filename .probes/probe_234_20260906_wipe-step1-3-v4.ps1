...
<#
  ...
  SCOPE         : gates G0-G7 (read-only) + stop our FOUR services + unregister them.
                  G0 is IDENTITY: which machine is this. It runs before anything is read.
  ...
  SAFETY        : without -Proceed the script ONLY measures and prints. Nothing is stopped.
                  THREE layers enforce this, because v2 had one and it failed:
                    1) Fin() always exits - it never returns control to the caller;
                    2) a sentinel immediately before STEP 2 aborts if -Proceed is absent;
...
function Fin($pass) {
    # LAYER 1: this function ALWAYS ends the script. A function that prints a verdict is not a
    # point of exit unless it actually exits. On 2026-09-06 the previous edition returned control
    # here on success and fell straight into STEP 2 - a guard without its 'else' branch is no guard.
...
# Seven gates guarded the STATE of the machine. None guarded its IDENTITY. On 2026-09-06 the
# operator's own workstation turned out to run RTMViewShell and RTMService under the same
# C:\RTMView\ paths, and commands meant for 234 were twice typed there by accident. Only the
# absence of the file saved it. This gate runs FIRST, before anything is read, and it exits -
# it does not warn.
# Reference values measured on 234 itself, 2026-09-06 14:37, by probe_234_20260906_identity.ps1.
$expName = "RTM"
$expUuid = "E9516FFB-3068-47EA-8860-6A9D764325E6"
$expMac  = "000D3AD3061B"
$expPres = Join-Path $OpsRoot "preserve_20260906_1230"
...
$okPres = (Test-Path $expPres)

Say ("  host name      : {0,-24} expected {1,-24} {2}" -f $gotName, $expName, $(if ($okName) { "ok" } else { "*** MISMATCH" }))
Say ("  VM UUID        : {0}   {1}" -f $gotUuid, $(if ($okUuid) { "ok" } else { "*** MISMATCH, expected $expUuid" }))
Say ("  MAC {0} present : {1}" -f $expMac, $(if ($okMac) { "ok" } else { "*** NOT FOUND" }))
Say ("  our footprint  : {0} -> {1}" -f $expPres, $(if ($okPres) { "ok" } else { "*** ABSENT" }))
Say ("  NEGCTL this machine is NOT the wrong one ('NOT-THE-SERVER') : {0}   (must be True)" -f ($gotName -ne 'NOT-THE-SERVER'))

if (-not ($okName -and $okUuid -and $okMac -and $okPres)) {
    Say ""
    Say "  *** G0 FAILED - THIS IS NOT SERVER 234. NOTHING WAS READ, NOTHING WAS TOUCHED."
    Say "  *** If you are on the operator's workstation, this box would have taken down a"
    Say "  *** running installation. That is exactly what G0 exists to prevent."
    Fin $false
}
...
# ---------------- LAYER 2: SENTINEL ----------------
# Execution must not be able to arrive here without -Proceed. If it does, something upstream
# failed to end the run - stop hard, before touching a single service.
if (-not $Proceed) {
    Say ""
    Say "*** SENTINEL: execution reached STEP 2 without -Proceed. This must never happen."
    Say "*** Nothing has been stopped. Aborting."
    Fin $false
}
