...
$src = "D:\Claude\Projects\RTM View Shell\.probes\probe_234_20260907_wipe-step4-v3.ps1"
...
$hashOk = ($sha -eq "280217937AD85537691EC0D3F97553A6AC999FF8F3338986ED009AF48D36D1EA")
Write-Host ""
Write-Host "===== BYTES ====="
Write-Host ("  bytes  : {0}   (expect 18299)" -f $b.Length)
...
# ---------------------------------------------------------------------------------------------
# VARIANT E - the one the earlier stands could not do.
# A, B, C and D all run with the GATE BLOCK CUT OUT and replaced by "$fail = 0" - that is how
# they isolate the question "does it stop?" from everything else. The cost is that they execute
# none of the gate logic, so on 2026-09-07 they were green while P2f silently checked nothing:
# its list of subtrees came out empty and it printed only its heading. A stand that removes the
# gates cannot test the gates. E runs the box with the gates INTACT, against a sandbox
# preserve_ tree, and asks only one thing: does P2f actually see its two subtrees and speak
# about both by name? The numbers will not match in a sandbox - that is fine and expected.
Write-Host ""
Write-Host "===== E - real box, GATES INTACT, sandbox preserve_ (does P2f actually check anything?) ====="
$sandE = Join-Path $env:TEMP ("gatelogic_{0}" -f [guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Force -Path (Join-Path $sandE "output") | Out-Null
$presE = Join-Path $sandE "preserve_20260906_1230"
foreach ($sub in @("ApplyService\env","ApplyService\files","Backup_20260829_1129","parallel_20260711_130140","configs\current")) {
    New-Item -ItemType Directory -Force -Path (Join-Path $presE $sub) | Out-Null
}
Set-Content -Path (Join-Path $presE "Backup_20260829_1129\manifest.txt")     -Value "AAAA|1|x.txt" -Encoding ASCII
Set-Content -Path (Join-Path $presE "parallel_20260711_130140\manifest.txt") -Value "BBBB|1|y.txt" -Encoding ASCII

$txtE = ($lines -join "`r`n")
$txtE = $txtE.Replace('$OpsRoot   = "C:\RTMView-Ops"', ('$OpsRoot   = "' + $sandE + '"'))
$txtE = $txtE.Replace('$Target    = "' + "C:\RTMView" + '"', ('$Target    = "' + $fakeTarget + '"'))
$txtE = $txtE.Replace('Remove-Item -LiteralPath $Target -Recurse -Force -ErrorAction SilentlyContinue',
                      'Write-Host "  [gate-logic] Remove-Item SUPPRESSED"')
$txtE = $txtE.Replace('$expName = "RTM"', ('$expName = "' + $env:COMPUTERNAME + '"'))
$txtE = $txtE.Replace('$expUuid = "E9516FFB-3068-47EA-8860-6A9D764325E6"', ('$expUuid = "' + $myUuid + '"'))
$txtE = $txtE.Replace('$expMac  = "000D3AD3061B"', ('$expMac  = "' + $myMac + '"'))
$fE = Join-Path $sandE "E_gatelogic.ps1"
[IO.File]::WriteAllText($fE, $txtE, (New-Object System.Text.UTF8Encoding($true)))
$isoE = (-not $txtE.Contains('Remove-Item -LiteralPath $Target'))
Write-Host ("  isolation applied : {0}   (must be True)" -f $isoE)
if (-not $isoE) { Write-Host "  STOP: isolation did not apply."; Remove-Item $sandE -Recurse -Force; exit 1 }

$re = RunIt $fE
$eKnows2  = $re.text.Contains("subtrees this box knows about : 2")
$eNames   = $re.text.Contains("Backup_20260829_1129") -and $re.text.Contains("parallel_20260711_130140")
$eP2fRan  = $re.text.Contains("P2f what was carried out")
Write-Host ("  P2f section reached                : {0}   (must be True)" -f $eP2fRan)
Write-Host ("  P2f sees BOTH subtrees (count = 2) : {0}   (must be True - this is the 2026-09-07 defect)" -f $eKnows2)
Write-Host ("  P2f names both subtrees in output  : {0}   (must be True)" -f $eNames)
Remove-Item $sandE -Recurse -Force -ErrorAction SilentlyContinue
...
Write-Host ""
if ($bom -and $nul -eq 0 -and $hashOk -and $errs.Count -eq 0 -and $aG0 -and $aStopped -and $aNoDel -and $bSent -and $bNoDel -and $cReached -and $dFailed -and $dNoP1 -and $eP2fRan -and $eKnows2 -and $eNames -and $sandboxFileSurvived) {
