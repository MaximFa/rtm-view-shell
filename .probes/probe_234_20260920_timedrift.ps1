<#
  PROBE  probe_234_20260920_timedrift.ps1
  UNIT   PR234-TIME-01 - WHERE the ~2 hour forward shift comes from. Fourth independent sighting:
         the newest RTSData_UserStatus row had an age of -7036 s, i.e. stamped in the future.
  WHAT   Three clocks side by side (machine local, machine UTC, database), the database's own time
         zone setting, and then the age of the newest row of EVERY timestamp column in the product
         tables - so the shift can be located instead of argued about.
  NEGATIVE HALF  A column WITHOUT the shift must be shown. If every column is shifted, that is also
         an answer, but it has to be printed, not implied: "everything is shifted" and "the query
         only looked at shifted things" are different statements.
  FIXES  NOTHING. Time in a live database is not corrected while its source is still unknown.
  WHERE  SERVER 234. READ-ONLY: no service touched, nothing written outside C:\RTMView-Ops\output\.
#>
...
$ReportPath = Join-Path $OutputDir ("234_{0}_timedrift.txt" -f $RunStamp)
...
Say "1  THE MACHINE'S OWN CLOCKS - both, plus the zone, so the later comparison has a reference"
$localNow = Get-Date
$utcNow   = $localNow.ToUniversalTime()
Say ("  machine local : " + $localNow.ToString('yyyy-MM-dd HH:mm:ss'))
Say ("  machine UTC   : " + $utcNow.ToString('yyyy-MM-dd HH:mm:ss'))
Say ("  offset        : " + [TimeZoneInfo]::Local.GetUtcOffset($localNow).TotalHours + " h   zone: " + [TimeZoneInfo]::Local.Id)
Say ("  w32tm source  : " + ((& w32tm /query /source 2>&1) -join ' '))
Rule

Say "2  THE DATABASE - clocks first, then every timestamp column"
Say  "  Nothing here is fixed. This run locates the shift; the fix depends on where it turns out to be."
$psql = $null
...
if ((-not $psql) -or (-not $dbPassword) -or (-not $dbName) -or (-not $dbPort)) { Say "  DATABASE SECTION : NOT MEASURED (-999)" }
  ...
  if (($okRc -ne 0) -or ($badRc -eq 0)) { Say "  DATABASE SECTION : NOT MEASURED (-999) - the client cannot tell success from failure" }
    ...
    $machineUtc = (Get-Date).ToUniversalTime().ToString('yyyy-MM-dd HH:mm:ss')
    Say ("  machine UTC handed to the database for comparison : " + $machineUtc)
    $queryText = @'
\timing off
SET statement_timeout = '60s';
SELECT 'db now()                 = ' || now()::text;
SELECT 'db now() AT TIME ZONE UTC= ' || (now() AT TIME ZONE 'UTC')::text;
SELECT 'db TimeZone setting      = ' || current_setting('TimeZone');
SELECT 'db - machineUTC seconds  = ' || round(EXTRACT(EPOCH FROM (now() AT TIME ZONE 'UTC') - :'mutc'::timestamp))::text;
SELECT 'server_version           = ' || current_setting('server_version');
SELECT '--- age of the NEWEST row per timestamp column (negative = stamped in the future) ---';
SELECT format('SELECT %L || '' age_s='' || coalesce(round(EXTRACT(EPOCH FROM (now() - max(%I))))::text, ''EMPTY'') FROM public.%I',
...
'@
    ...
    $queryErr = Join-Path $OutputDir ("234_{0}_timedrift.err.txt" -f $RunStamp)
    & $psql -v ON_ERROR_STOP=1 -v mutc="$machineUtc" -h 127.0.0.1 -p $dbPort -U $dbUser -d $dbName -At -o $queryOut -f $queryFile 2> $queryErr
...
Say "VERDICT"
Say  "  This run DECIDES nothing and FIXES nothing. It prints three clocks and every timestamp column,"
Say  "  so the shift can be located: a column with age >= 0 next to columns with negative age means the"
Say  "  shift belongs to the writer of those columns, not to the machine or the database as a whole."
Say  "  If EVERY column is negative, the machine/database clocks are the first suspect and the writer"
Say  "  the second - and that reading is in the three lines of section 1 against section 2."
Say "===== END-OF-RUN MARKER: TIMEDRIFT-COMPLETE ====="
Finish $true
