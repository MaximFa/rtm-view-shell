...
if (-not (Test-Path $Manifest)) { Say "  *** STOP: evacuation manifest missing"; $fail++ }
    ...
    Say ("  evacuation: {0} files, not in manifest {1}   (expect 884 / 0)" -f $files.Count, $unk)
...
Say ("  preserved evacuation files  : {0}   (must be 884)" -f @(Get-ChildItem $Preserve -Recurse -File -ErrorAction SilentlyContinue | Where-Object { $_.FullName -notlike "$AppPres*" -and $_.Name -ne 'ZZZ_control.txt' }).Count)
