...
Say ("  -> ShellPort      = {0} / ShellHttpsPort = {1}   (DERIVED the same way)" -f $httpPort, $httpsPort)
...
if ($fail -gt 0) { Say "  *** STOP - a missing value would become a placeholder on the machine."; Fin $false }

Say ""
Say "  NOTE: Fqdn and the two ports are DERIVED from the Kestrel urls, because the saved Shell"
Say "        config carries no Fqdn/CertSubject keys. Read them above and confirm they are right"
Say "        before running with -Proceed. CertSubject is NOT passed - the installer keeps its default."
...
Say ("      -Fqdn {0} -ShellPort {1} -ShellHttpsPort {2} ``" -f $fqdn, $httpPort, $httpsPort)
