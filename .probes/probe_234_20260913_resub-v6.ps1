...
<#
  PROBE 234 / resub-v6   BOX B of two. ONE run closing BOTH analyses - shell (PR234-SHELL-RESUB-01) and backend
...
$outf  = Join-Path $OutDir ('234_' + $stamp + '_resub-v6.txt')
...
# Print the deployed Serilog settings, because the other candidate explanation for a silent file is LEVEL
# filtering, and it must be ruled in or out from the report rather than from the repository. RECV
# updateUserGrid is written with LogInformation (RtmRelayService.cs:349,696) and the repo config has
# MinimumLevel.Default = Information, so in the repo shape it reaches the file - but the deployed file is
# the operator's and may differ. No secret lives in this section; it prints as is.
try {
    $lvl = [regex]::Matches($shJson, '"(MinimumLevel|Default|Override|Name|rollingInterval|retainedFileCountLimit)"\s*:\s*("[^"]*"|\{[^}]*\}|[0-9]+)')
    Say ('  deployed Serilog-related settings found : {0}' -f (Safe-Count $lvl))
    foreach ($g in $lvl) { Say ('      ' + $g.Value) }
} catch { Say ('  *** could not read the Serilog settings : {0}' -f $_.Exception.Message) }
