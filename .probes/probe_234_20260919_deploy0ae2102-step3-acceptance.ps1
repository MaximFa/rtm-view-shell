...
<#
  PROBE  probe_234_20260919_deploy0ae2102-step3-acceptance.ps1
...
$InstallEndedUtc     = [datetime]::Parse('2026-09-18T23:04:00Z').ToUniversalTime()
...
Say "1  SERVICES - running, and restarted BY THIS FLIGHT (a service that never went down proves nothing)"
$twilioRunning = $false
foreach ($serviceName in @('RTMService','RTMViewShell','RTMTwilio_1')) {
  ...
  $startedText = 'n/a'
  $startedAfterInstall = $false
  if ($service.ProcessId -gt 0) {
    $process = Get-Process -Id $service.ProcessId -ErrorAction SilentlyContinue
    if ($process) {
      $startedUtc = $process.StartTime.ToUniversalTime()
      $startedText = $startedUtc.ToString('u')
      $startedAfterInstall = ($startedUtc -gt $InstallEndedUtc.AddMinutes(-5))
    }
  }
  ...
  Record ($serviceName + " restarted by this flight") $startedAfterInstall ("process start " + $startedText + " vs install ~" + $InstallEndedUtc.ToString('u'))
  if ($serviceName -eq 'RTMTwilio_1') { $twilioRunning = $isRunning }
}
