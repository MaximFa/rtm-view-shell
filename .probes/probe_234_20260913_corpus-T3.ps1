...
function Fin($pass) {
    ...
    # print a path only if the file actually exists - v1 printed three paths and then stopped at a gate,
    # sending the operator to fetch files that had never been written.
    foreach ($f in @($engOut, $shOut, $posOut)) {
...
if ($starts.ContainsKey('RTMViewShell')) {
    # THE PREDICATE IS "the Shell started BEFORE the window", not "the Shell start equals a time I typed".
    # v1 of this gate compared against a whole second and went red on 400 milliseconds: Process.StartTime
    # carries fractions, 22:15:05.4xx is greater than 22:15:05.000, and a correct run was stopped by its
    # own instrument. A predicate written against a typed constant also rots the moment anything restarts
    # again; this one is measured on both sides and cannot.
    $shellStart = $starts['RTMViewShell']
    $predates = ($shellStart -lt $WINSTART)
    Say ('  shell started {0} ; window starts {1} ; shell PREDATES the window -> {2}   expected True' -f `
         $shellStart.ToString('yyyy-MM-dd HH:mm:ss.fff'), $WINSTART.ToString('yyyy-MM-dd HH:mm:ss.fff'), $predates)
    Say ('  (informational, not a gate) pinned shell start was {0}' -f $SHELLPIN.ToString('yyyy-MM-dd HH:mm:ss'))
    if (-not $predates) {
        Say '  *** THE SHELL RESTARTED INSIDE OR AFTER THE WINDOW. The subscription under test would be a'
        Say '  *** FRESH one, and its numbers would repeat the empty run. Nothing is written.'
        Fin $false
    }
}
