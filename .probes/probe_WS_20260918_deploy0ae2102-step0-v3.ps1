<#
  PROBE  probe_WS_20260918_deploy0ae2102-step0-v3.ps1
...
try {
  ...
  $selfName = 'probe_WS_20260918_deploy0ae2102-step0-v3.ps1'
    ...
    foreach ($dir in $prior) {
      ...
      if (-not (Test-Path -LiteralPath $mark)) {
        ...
        if ($txt -like ("*" + $selfStem + "*")) {
          Say ("  " + $dir.Name + "  -> MY OWN previous run of this step (v1 or v2). NOT TOUCHED; this run makes a fresh stamped clone. Delete it by hand when you no longer want it.")
        } else {
