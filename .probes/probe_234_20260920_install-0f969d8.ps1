<#
  PROBE  probe_234_20260920_install-0f969d8.ps1
  BASED  ON probe_234_20260919_install-d702e2d.ps1 - the run that installed d702e2d without a surprise.
  UNIT   install 20092026.1110_Shell.zip, built from 0f969d8: five fixes under one package.
  WHAT   The delta touches three widgets and all three .resx dictionaries.
         THE SATELLITES MUST MOVE - the inverse of the previous flight and the main predicate here.
         A satellite still equal to its BEFORE size means the translations did not install.
  OUTAGE TWO-SERVICE OUTAGE. The updater stops and starts BOTH the engine and the adapter regardless
         of -Skip flags. Previous install measured 43.7 s, plus ~16 s until the engine raises the adapter.
...
param(
  [string]$PackagePath = 'C:\RTMView-Ops\incoming\20092026.1110_Shell.zip'
)

$ErrorActionPreference = 'Continue'
$PinnedZipSha  = '85A6F2706D62C51E8486DFB9E7E93538E65EB7960FF17B65FA2991761A60903D'
$PinnedWebDll  = 'E9D27996B41BB797A1C67EA3DEEE1BAEF46A0D5029E9E904792E07BF67973560'
$PinnedDataSys = '24F0BFACE0A0F7D076DF7CFB2CF98933B8FC7F178166FD680567440FF04DDE43'
$PinnedCommit  = '0f969d8'
# the floor we fall back to: the package this machine runs today, and the proof it is physically here
$RollbackZip    = 'C:\RTMView-Ops\incoming\19092026.1544_Shell.zip'
$RollbackZipSha = 'A40603C2C94485185AACE346E4514F922900464345F0FC81D75105B0437D7AA9'
$RollbackWebDll = '2D342BE2E2A6BBCF7BE0DA0CA1D65237122AC5D3DC61858CABBBC5EBF395D531'
...
# Measured ON THIS MACHINE at 11:26 today, before the install (probe predeploy-state-pkg3):
$SatelliteBeforeBytes  = @{ 'he-IL' = 73728 ; 'ru-RU' = 83968 ; 'en-US' = 67584 }
# Measured INSIDE THE PACKAGE on the station during the build:
$SatellitePackageBytes = @{ 'he-IL' = 74752 ; 'ru-RU' = 84992 ; 'en-US' = 68096 }
...
$ReportPath = Join-Path $OutputDir ("234_{0}_install-0f969d8.txt" -f $RunStamp)
...
Say "  GATE GREEN - the bytes on this server are the bytes built from 0f969d8"
...
Say "===== END-OF-RUN MARKER: INSTALL-0F969D8-COMPLETE ====="
Finish $stepOk
