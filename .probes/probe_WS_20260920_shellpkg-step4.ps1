<#
  PROBE  probe_WS_20260920_shellpkg-step4.ps1
  BASED  ON probe_WS_20260920_shellpkg-step3.ps1 (that run built 0f969d8 and is the proven shape).
  UNIT   FOURTH Shell package: filter popup closes on a dashboard click (CLOSE-SCOPE-01), the screen
         editor markup localised (KEYLEAK step 2) and 19 editor keys into three dictionaries.
         Pin comes from the coordinator as 251ca40 and is RE-TAKEN HERE, at build time.
  WHERE  WORKSTATION ONLY. Server 234 is NOT touched: no network to it, no copy, no install.

  THE PREDICATE THAT MATTERS, AND IT IS INVERTED ON PURPOSE
    This delta edits all three .resx, so every satellite MUST MOVE. Equality with what 234 carries
    is RED: it would mean the translations never reached the package.
    The 'on 234' numbers below are NOT the previous flight's. They were measured on the machine
    at 2026-09-20 20:32 local, report 234_20260920_203215_predeploy-state-pkg4.txt, and they are
    the ONLY sizes this run may compare against. The previous flight's numbers (73728 / 83968 / 67584)
    belong to a package that is no longer installed; inheriting them would compare against a machine
    state that no longer exists.
    No expected SIZE for the package is stated here. Only the DIRECTION is: larger, by a multiple of
    512, on all three. Nineteen keys were added to each dictionary and none removed [measured on the
    station: en-US 938 -> 957, he-IL 938 -> 957, ru-RU 938 -> 957].
...
$WITNESSES  = @(
  ...
  'src/CcDashboard.Web/Services/FilterPopupCoordinator.cs',
...
$DEPLOYED   = '0f969d8'   # what 234 runs now - the delta is measured against THIS, not against the branch
# Satellite sizes measured ON 234 at 2026-09-20 20:32 local, by
# probe_234_20260920_predeploy-pkg4-0f969d8.ps1, report 234_20260920_203215_predeploy-state-pkg4.txt.
# These are the sizes of the package that is RUNNING NOW (0f969d8). They are re-measured each flight
# and never carried over: the previous flight's 73728/83968/67584 describe a package that has since
# been replaced.
# EXPECTATION FOR THIS PACKAGE, STATED BEFORE THE MEASUREMENT: every one of the three DIFFERS from
# these numbers, and differs UPWARDS, because this delta adds 19 keys to each of the three .resx
# files and removes none. Equality is a FINDING, not a pass. A SMALLER satellite is also a finding.
$ON_SERVER_BYTES = @{
  'he-IL' = 74752
  'ru-RU' = 84992
  'en-US' = 68096
...
$logPath = Join-Path $measDir ("WS_{0}_shellpkg-step4.txt" -f $stamp)
...
try {
  Say ("SHELL PACKAGE STEP 4  " + (Get-Date -Format 'u') + "   host=" + $env:COMPUTERNAME + "   user=" + $env:USERNAME)
  ...
  $selfName = 'probe_WS_20260920_shellpkg-step4.ps1'
  $selfStem = 'shellpkg-step4'   # only clones carrying THIS step's own mark are ever removed
  ...
  # ---- PIN 1: clean clone, HEAD == the pin re-taken above --------------------
  ...
  $originTxt = @(
    ...
    ("commit  : " + $PIN_COMMIT),
    ("unit    : fourth Shell package (filter close scope + screen editor l10n), by devops-0919")
  ) -join "`r`n"
  ...
  Say "PIN 3  content witnesses - the SIX product files of this delta, blob by blob"
  $pinWitness = $true
  ...
  Say ("  PIN 3 " + $(if ($pinWitness) { 'GREEN  all six blobs equal' } else { 'RED    a blob differs - the builder would read a tree I never checked' }))
  Line
  ...
  try {
    ...
    foreach ($culture in @('he-IL','ru-RU','en-US')) {
      ...
      $delta       = $pkgBytes - $serverBytes
      $moved       = ($delta -gt 0)
      if ($moved) { $satellitesMoved++ }
      $reading     = if ($delta -gt 0) { 'as expected: GREW (+' + $delta + ' B, multiple of 512: ' + (($delta % 512) -eq 0) + ')' }
                     elseif ($delta -eq 0) { 'FINDING: identical to 234 - the new keys are NOT in this package' }
                     else { 'FINDING: SMALLER than 234 by ' + [math]::Abs($delta) + ' B - keys were lost, not added' }
    ...
    Say  "  NEGATIVE HALF is real and was taken on 19.09: the same comparison on the d702e2d package"
    Say  "  printed all three as EQUAL, because that delta carried no .resx. The check can say both."
    Say  "  It can now also say a THIRD thing - smaller - which equality alone could not distinguish."
