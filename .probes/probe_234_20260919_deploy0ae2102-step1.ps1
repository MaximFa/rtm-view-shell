...
<#
  PROBE  probe_234_20260919_deploy0ae2102-step1.ps1
...
function H($p) { if (Test-Path -LiteralPath $p) { (Get-FileHash -LiteralPath $p -Algorithm SHA256).Hash } else { 'ABSENT' } }
...
if (Test-Path -LiteralPath $shCfg) {
  ...
  else { Say "  Shell conn Password     = NOT FOUND - the database section below will be a gap, not a pass" }
}
Line
