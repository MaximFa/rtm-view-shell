...
$target = "D:\Claude\Projects\RTM View Shell\.probes\probe_234_20260908_install-v3.ps1"
...
$hashOk = ($sha -eq "4E1BD0A8AAFB1BE6CDF4B2A1F6E4967B5A5FC3B3C7D9D98A531F7CC4685F7AA3")
Write-Host ("bytes {0} (expect 17529)   BOM {1}   NUL {2}   hash matches {3}" -f $b.Length, $bom, $nul, $hashOk)
...
$certPass  = $txt.Contains('$args += @("-CertSubject",$certSubject)')
# Looks for a CONSTRUCTED value, not for a mention. The box explains in a comment that the
# certificate in the store is CN=*.insightense.com - that is documentation, not a guess.
# The defect to catch is building the value: "CN=" + something, or "CN=$var".
$noCnGuess = -not ($txt -match '(?i)[''"]CN=\s*[''"]?\s*\+' -or $txt -match '(?i)"CN=\$')
$hangGuard = $txt.Contains('the installer would stop at Read-Host and hang')
Write-Host ""
...
Write-Host ("  -CertSubject actually PASSED           : {0}   (must be True - empty means the installer goes interactive)" -f $certPass)
Write-Host ('  no hand-built CN= string anywhere      : {0}   (must be True - the config value is verbatim)' -f $noCnGuess)
Write-Host ("  refuses to run if Fqdn/CertSubject empty: {0}   (must be True)" -f $hangGuard)
...
Write-Host ""
if ($negOk -and $bom -and $nul -eq 0 -and $hashOk -and $freshDb -and $port -and $no5432 -and $oneShot -and $noReturn -and $dumpByExt -and $redisPass -and $masked -and $noRawPrint -and $redact -and $certPass -and $noCnGuess -and $hangGuard -and $maskWorks -and $errs.Count -eq 0) {
