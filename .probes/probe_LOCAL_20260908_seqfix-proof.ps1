...
<#
  PROBE LOCAL / seqfix-proof   -   BEHAVIOURAL proof of the step-7 fix. THIS ONE WRITES,
  but only into a throwaway database it creates itself and drops at the end.
  WHERE IT RUNS : the BUILD machine, PostgreSQL 18 on port 5433. It REFUSES to run on server 234.
...
$outf  = Join-Path $OutDir ("LOCAL_{0}_seqfix-proof.txt" -f $stamp)
...
Say "===== WHERE IT RUNS ====="
Say "  the BUILD machine. PostgreSQL 18 on port 5433. Server 234 is NOT touched."
...
Say "===== G1  instrument can fail, and the target name is FREE (gate) ====="
...
$after = RunSql $TestDb "SELECT 'RTSGrid_Cell seq last_value=' || last_value::text FROM public.\"RTSGrid_Cell_CellId_seq\";" $true
...
$ins = RunSql $TestDb "INSERT INTO public.\"RTSGrid_Cell\" (note) VALUES ('insert after resync');" $true
...
Say "===== END-OF-RUN MARKER: SEQFIX-PROOF-COMPLETE ====="
