...
<#
  PROBE 234 / inst14-D                PR234-INST-14 behaviour + A on Windows PowerShell 5.1.
...
$REPORT = Join-Path $OUT ('234_' + $STAMP + '_inst14-D.txt')
...
function Run-Child($tag, $scriptPath, $flag) {
    ...
    $log = Join-Path $OUT ('234_' + $STAMP + '_inst14-' + $tag + '-child.txt')
...
'@
    ...
    $skip = @(($t -split "`r?`n") | Where-Object { $_ -like '*Drift gate SKIPPED*' })
    return [pscustomobject]@{ Rc=$p.ExitCode; Text=$t; Skip=($skip -join ' || '); SkipCount=$skip.Count }
}

Say '===== PROBE 234 / inst14-D ====='
...
Say 'EXPECT ALL  : exactly 1 SKIPPED line per run, "Not installed" for both fake names, NO "Stopped:" line, real services pid+StartTime before == after'
Say ''
...
foreach ($k in @('OLD','NEW')) { foreach ($flag in @('SkipDrift','ForceDeploy','SkipDriftGate')) {
    ...
    $hit = ($r.SkipCount -eq 1) -and (Has $r.Skip $want[$key])
    $got[$key] = $r.Skip.Trim()
    Say ('  {0,-18} rc={1} POSCTL {2} | SKIPPED lines {3} | "{4}" | want "{5}" -> {6} | Not installed x2 {7} | no Stopped {8}' -f $key, $r.Rc, $pos, $r.SkipCount, $r.Skip.Trim(), $want[$key], $hit, $ni, $noStop)
