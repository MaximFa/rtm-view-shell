...
<#
  PROBE LOCAL / installer-89   -   READ ONLY. Nothing is built, installed or written into the clone.
...
$outf  = Join-Path $OutDir ("LOCAL_{0}_installer-89.txt" -f $stamp)
...
$checks = @(
    ...
    [pscustomobject]@{ N=3; What="data.sys is in the RTM preserve list of the INSTALLER";
                       Pat='$preserveRTM = @("data.sys"'; ExpNewI=1; ExpNewU=1; ExpOldI=0; ExpOldU=1; DiscrFile="Install" },
...
foreach ($c in $checks) {
    ...
    if ($c.N -eq 3) { Say "  note: Update carries this line in BOTH revisions (1 and 1) - that half is NOT a discriminator and is not counted as one" }
...
Say "===== END-OF-RUN MARKER: INSTALLER-89-COMPLETE ====="
