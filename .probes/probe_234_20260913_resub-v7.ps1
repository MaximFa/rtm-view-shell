...
<#
  PROBE 234 / resub-v7   BOX B of two. ONE run closing BOTH analyses - shell (PR234-SHELL-RESUB-01) and backend
...
$outf  = Join-Path $OutDir ('234_' + $stamp + '_resub-v7.txt')
...
try {
    ...
    foreach ($m in [regex]::Matches($shJson, '"path"\s*:\s*"([^"]+)"')) {
        ...
        Say ('  SHELL Serilog declares path : {0}' -f $declared)
        if ([IO.Path]::IsPathRooted($declared)) {
            Say ('      ROOTED - resolves to {0}' -f $declared)
            [void]$serilogPaths.Add([IO.Path]::GetDirectoryName($declared))
        } else {
            # A RELATIVE Serilog path resolves against the PROCESS working directory, and a Windows
            # service starts with its working directory at %SystemRoot%\System32 unless the service
            # definition sets one. So the live log of a service does NOT land beside its exe - it lands
            # under System32\logs. v5 resolved the relative path only against the install directory and
            # so measured leftovers from interactive runs: newest there was 5 days old and 538 bytes.
            # [operator, 2026-09-13]: the log IS written to C:\Windows\System32\logs, and this config
            # used to hold an ABSOLUTE path - it was overwritten by an install. The 30 files in
            # C:\Logs\RTMViewShell ending 2026-09-06, exactly retainedFileCountLimit, are that old
            # absolute target. Both bases are resolved here and both are printed.
            Say '      RELATIVE - resolved against BOTH the install dir and the service working dir'
            foreach ($base in @($shellDir, [Environment]::SystemDirectory)) {
                $full = Join-Path $base $declared
                Say ('        against {0}  ->  {1}' -f $base, $full)
...
# v6 PRINTED A PRODUCTION PASSWORD into its report. My defect, and a bad one: I scanned the WHOLE config
# with one regex for '"Default"' among other keys, and ConnectionStrings:Default is also called "Default",
# so the database connection string - password included - went into the artifact. The standing norm is the
# coordinator's of 2026-09-05: a config is reported BY KEY with values MASKED, and only a key shown to be
# non-secret may have its value printed. A blanket regex over a config file can never satisfy that,
# because it cannot know what it will match. Replaced by an explicit ALLOWLIST navigated through the
# parsed object: nothing outside Serilog is touched, and nothing is printed that was not named here.
try {
    $shObj = $shJson | ConvertFrom-Json
    $ser = $shObj.Serilog
    if ($null -eq $ser) {
        Say '  deployed config has NO Serilog section'
    } else {
        Say ('  Serilog MinimumLevel.Default : {0}' -f $ser.MinimumLevel.Default)
        if ($ser.MinimumLevel.Override) {
            foreach ($ovName in @($ser.MinimumLevel.Override.PSObject.Properties.Name)) {
                Say ('  Serilog MinimumLevel.Override[{0}] : {1}' -f $ovName, $ser.MinimumLevel.Override.$ovName)
            }
        }
        $sinkIdx = 0
        foreach ($sink in @($ser.WriteTo)) {
            Say ('  Serilog sink[{0}].Name : {1}' -f $sinkIdx, $sink.Name)
            foreach ($argName in @('path','rollingInterval','retainedFileCountLimit','fileSizeLimitBytes','shared')) {
                if ($sink.Args -and $null -ne $sink.Args.$argName) {
                    Say ('      Args.{0} : {1}' -f $argName, $sink.Args.$argName)
                }
            }
            $sinkIdx++
        }
        Say '  (only the keys named above are printed; no other part of the operator config is read out)'
    }
} catch { Say ('  *** could not read the Serilog section : {0}' -f $_.Exception.Message) }
$shRoots = @(@($serilogPaths) + $shRoots + @((Join-Path ([Environment]::SystemDirectory) 'logs')) |
