...
| `коорд: пуш` | coordinator | First runs the L-SC-29 preflight (branch / explicit-adds / restore-truncated / no Export-All / no secrets); no quorum or failed preflight → do NOT push. Verify quorum (every active session READY, valid_for == frozen set) → reply with the CC command (`Выполни задачу из файла tools/cc_prompt_push_barrier.md`). If no quorum: who is missing. **Policy (L-SC-29 decision):** the preflight is CANONICAL — bake it into `tools/cc_prompt_push.md` (and its caller `tools/cc_prompt_push_barrier.md`) so it cannot go stale per-barrier; retire one-off push prompts after use. |
