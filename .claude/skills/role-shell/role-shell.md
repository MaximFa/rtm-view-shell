...
### §A · 2026-09-19 · ФОРМА ОБРАЩЕНИЯ К ОПЕРАТОРУ БЕРЁТСЯ ИЗ КАНОНА, А НЕ ИЗ МОЕГО ВЫВОДА
[со слов оператора: 2026-09-19] Я выдал ран-бокс вольным пересказом; оператор поправил — я починил
СОДЕРЖАНИЕ промпта и следующей же выдачей снова промахнулся ФОРМОЙ, потому что взял её из своего
собственного ответа предыдущего хода, а не из нормы. Канон: NORM-CUR-02 / CLAUDE.md §0.7 — промпт
для CC подаётся ТОЛЬКО одной строкой `Выполни задачу из файла tools/<имя>.md`; инлайн-промпта в чате
не существует, любые мои дописки поверх строки — вторая, неверсионированная копия задания.
**Механизм промаха, а не небрежность:** поправка закрывает названный дефект, и на радости починки
я перестаю сверять соседнее. Мой же урок 2026-09-15 («сверяться с каноническим рунбуком, а не с
представлением») не сработал именно потому, что я применял его к содержанию, а не к форме подачи.
**Предикат перед КАЖДЫМ сообщением оператору, где есть код-бокс или бокс:** назвать параграф канона,
задающий форму этого обращения, и открыть его. Не назвал параграф — не отправлять.
**Отдельно:** после чужой поправки следующий ход сверяется ЦЕЛИКОМ, а не только в точке поправки.
SOURCE: оператор 2026-09-19 («так будет каждый раз?»); CLAUDE.md §0.7 NORM-CUR-02. status: active

### §A · 2026-09-19 · ПРОИЗНЕСЕНИЕ НАМЕРЕНИЯ ВМЕСТО ДЕЙСТВИЯ — УРОН ПРОИЗВОДСТВУ
[со слов оператора: 2026-09-19] Заявленное намерение не является работой. «Пишу промпт», «сниму замер»,
«занесу в `§B`», «сделаю при следующем заходе» — это обещание, и если ход кончается им, то ход потрачен,
а продукт не сдвинулся. Цена не в стиле: координатор и оператор планируют очередь по моим словам, и
обещание встаёт в неё как сделанное.
**Правило:** если действие возможно СЕЙЧАС и не ждёт чужого хода — оно делается в ЭТОМ ходе, а отчёт
описывает сделанное. Если действие невозможно (нужен §4, бокс оператору, чужая правка) — называется
именно БЛОКЕР и чей ход, а не «я собираюсь».
**Предикат на себя перед отправкой сообщения:** есть ли в нём глагол будущего времени о том, что я мог
сделать прямо сейчас? Есть — не отправлять, сделать и переписать.
Собственный случай, из-за которого правило и записано: в том же сообщении, где я разбирал 44 литерала,
я закончил словами «пишу промпт, принесу на §4» — и отправил, не написав его.
SOURCE: оператор 2026-09-19. status: active
...
> **§D — только адреса материалов.** Секция не грузится при ините: правило, положенное сюда,
> до роли НЕ ДОХОДИТ — оно не забывается, оно не читается. Всё, что правит поведение, живёт
> в `§A` (грузится всегда) или в `§B` (уроки с датой и источником).
> Норма стандарта, внесена 2026-09-19; перенос 19 блоков из-под `§D` сделан shell-0919b тем же днём.
> Формулировка намеренно без модальных слов: иначе сенсор считает собственный текст нормы.
