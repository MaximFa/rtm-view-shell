...
## PR234-SIGN-LEAK-01 · ПОДПИСИ АССИСТЕНТА В СООБЩЕНИЯХ КОММИТОВ · OPEN · заведён coordinator-0919b 2026-09-24

### Запрет, который нарушен
[со слов оператора, действует постоянно] «NO push, ветку не двигать. **Подписи ассистента в коммитах
запрещены.**»

### Как найдено
Не искал. Наткнулся, проверяя §4-приёмку `08a548d` (снос `.filter-overlay`): в моём обычном блоке
контроля стоит строка «подписи ассистента: ожидание 0». Впервые за смену она вернула НЕ ноль.

### Измерено
```
[измерено coordinator-0919b, 2026-09-24, ОБЛАСТЬ: git log v3, последние 400 коммитов,
 единица — коммиты; игла регистронезависимая: co-authored-by | claude-session | anthropic]
подписанных НЕпушенных (после ed3e292) : 35
подписанных УЖЕ пушенных (до ed3e292)  : 11
всего непушенных в ветке               : 113
NEGCTL: та же игла по 6902e71 (мой коммит) -> 0 — предикат способен вернуть ноль
POSCTL: 08a548d -> 2 строки, предъявлены дословно:
        «Co-Authored-By: Claude Opus 4.5 <noreply@anthropic.com>»
        «Claude-Session: https://claude.ai/code/session_018JZ...»
```

### Что это значит, а не как выглядит
Подпись попадает в сообщение **не рукой роли**: роли пишут тело письма, а трейлер дописывается
средой исполнения при коммите. Поэтому запрет, объявленный оператором, ни одной ролью не оспаривался
и всеми соблюдался в тексте — и всё равно нарушался в объекте. **Это отказ класса «выполнено в
докладе, не выполнено в хранилище»** — ровно тот класс, против которого построена вся наша
дисциплина объектного стора, и он прошёл мимо неё, потому что ни один прибор колонии не мерил
сообщение коммита.

Добавлено в мой блок контроля навсегда: **у каждой приёмки коммита есть строка «подписи: ожидание 0».**

### Граница — называю до предложений
- **35 непушенных** переписываются без последствий для кого-либо: ветка не двигалась, чужих копий нет.
- **11 уже пушенных** переписать нельзя иначе как насильственным пушем, а push запрещён отдельным
  словом оператора. **Их я предлагаю оставить как есть и зафиксировать фактом, а не чистить.**
- Что подпись НЕ попала в содержимое файлов (а только в сообщения) — **здесь НЕ показано**,
  отдельный замер, не делал.

### Решения нет
Переписывание 35 сообщений — необратимое действие над историей. По норме согласования это
требует явного слова оператора. Вопрос ему поставлен 2026-09-24.


### Решение оператора 2026-09-24
[со слов оператора: 2026-09-24] «нет, оставляем как есть».
История не переписывается: ни 35 непушенных, ни 11 пушенных. Чистки не будет.
Всем ролям разослано, находкой заново не заводится.

**Предмет остаётся OPEN не потому, что есть работа, а потому, что закрытие — только явным словом
оператора.** Работы под ним нет ни у одной роли. Закрывается словом «закрываем».

### Что добавлено навсегда вместо чистки
В блок контроля координатора при каждой приёмке коммита — строка «подписи: ожидание 0».
Она и нашла этот предмет. До сегодняшнего дня ни один прибор колонии не мерил СООБЩЕНИЕ коммита:
мерили содержимое файлов, блобы, хеши — всё, кроме текста, который мы пишем сами.
