...
## OPS-BACKUP-234 — на 234 НЕТ задания резервного копирования БД · РЕШЕНИЕ ОПЕРАТОРА 2026-09-05: ЖДЁМ ЗАКРЫТИЯ CONVERGE

- **Находка:** `devops-0905` при ините, 2026-09-05. На боевом 234 задания резервного копирования БД
  нет ВОВСЕ. Смежно: подтверждения ротации пароля `postgres` на 234 нет — оператор снял вопрос с
  очереди, а снял и выполнил это разные вещи.
- **Обстоятельство, из-за которого это не рядовой пункт:** незакрытый `BINDING 2026-08-29T08:34Z`
  (`converge234_transfer`), `RUN 3` начат и не закрыт на боевой базе `234:5433`. То есть окно без
  бэкапа совпадает с окном незавершённой операции над данными.
- **РЕШЕНИЕ ОПЕРАТОРА 2026-09-05: не заводить сейчас, ждать закрытия converge.** Зафиксировано
  куратором со слов оператора. Пункт не закрыт и не отложен бессрочно — он ОЖИДАЕТ СОБЫТИЯ.
- **Условие пробуждения (чтобы пункт не замолчал):** закрытие `BINDING 08:34Z` — в тот же час, когда
  devops помечает его закрытым, OPS-BACKUP-234 поднимается оператору первым предметом. Ответственный
  за подъём — куратор; напоминание держится здесь, а не в чьей-либо памяти.
- **Чего этот пункт НЕ означает:** решение «ждём» относится к ЗАВЕДЕНИЮ регулярного задания. Оно не
  разрешает и не запрещает разовый дамп перед опасным шагом — это решение владельца шага по месту.
- status: **WAITING-ON-EVENT** (не open, не closed; закрытие — только словом оператора)


## PR234-SHELL-CFG-01 — на 234 в конфиге Shell отсутствуют три ключа из четырёх, путь логов относительный · РЕШЕНИЕ ОПЕРАТОРА 2026-09-08: ПОСЛЕ, НЕ СЕЙЧАС

- **Заявлен:** `devops-0908b`, замер `.measurements/234_20260908_123548_acceptance8.txt`, пункт §8.6.
  Вопрос координатору задан 2026-09-08T18:55Z, ответ дан 2026-09-08 после смены инкарнации.
- **Факт, числом:** `DefaultTenantSlug` — KEY ABSENT; `MetricsApply BaseUrl` — KEY ABSENT;
  `RtmRelay level override` — KEY ABSENT; `Serilog file path` = `'logs/log-.txt'` — ОТНОСИТЕЛЬНЫЙ.
- **Атрибуция (важна, чтобы не искали виноватого в свежей установке):** это НЕ регресс установки
  08.09. Тот же конфиг мерился в 11:18, ДО установки — `TenantSlug` уже отсутствовал. Установщик в
  11:26 доложил `Preserved: appsettings.json (package version ignored)` и сохранил то, что было.
  **Сохранение сработало; сохранять было уже нечего.** Ключи потеряны раньше — ночная установка 01:40
  либо заход 6 сентября. Когда именно — НЕ УСТАНОВЛЕНО, версий не строим.
- **Наблюдаемые следствия сейчас:** вход по `localhost` без слага не работает (оператор входит по
  `insightense.com`, там слаг берётся из имени хоста — потому и не мешало); `MetricsApply.BaseUrl`
  указывать некуда, `RTMApplyService` на машине отсутствует вовсе; логи Shell при CWD службы уезжают
  не туда, где их ищут.
- **Почему это не делается походя:** правка живого конфига на боевой машине + перезапуск Shell.
- **РЕШЕНИЕ ОПЕРАТОРА 2026-09-08: ПОСЛЕ. Сейчас есть более насущные задачи.** Пункт не закрыт и не
  отменён — отложен по приоритету.
- **Как делать, когда дойдёт ход (записано, чтобы не восстанавливать заново):** значения брать из
  `preserve_install_20260908_112533` и более ранних снимков; предъявлять чтением с диска ДО и ПОСЛЕ,
  с guard'ом, без BOM; путь Serilog сделать абсолютным; после правки — перезапуск и живость ПАРОЙ
  (`/health` 200 И пайп), а не по одному признаку.
- **Условие пробуждения:** поднимается первым предметом, когда закроем §8.9 (шесть проверок
  установщика) — или раньше, если ключи начнут мешать делу. Напоминание держится здесь, а не в
  чьей-либо памяти.
- status: **DEFERRED-BY-OPERATOR** (не open-в-работе, не closed; закрытие — только словом оператора)


## PR234-TIME-01 — метки `RTSData_Interaction` пишутся на два часа ВПЕРЁД относительно времени базы · ЗАВЕДЁН 2026-09-08, ЧИСЛО ЕСТЬ, ПРИЧИНА НЕТ

- **Заявлен:** `devops-0908c`, замер `.measurements/234_20260908_155038_state-3.txt`, прогон 15:50 локального.
  Находка ПОБОЧНАЯ — её не искали, прибор мерил свежесть потока.
- **Факт, числом:** `now() - max("UpdateTime")` по `RTSData_Interaction` = **−7197 с** (минус два часа).
  По `RTSData_UserStatus` в том же прогоне возраст нормальный (5 с и 1 с). **Расхождение не общее для
  потока, а именно у `Interaction`.**
- **Что это НЕ:** это не «часы сервера убежали» — тогда уехали бы обе таблицы. Гипотеза о смешении
  локального времени и UTC при записи названа автором ЯВНО как гипотеза; в реестр она идёт как версия,
  а не как причина. Причина НЕ УСТАНОВЛЕНА.
- **Практические следствия, уже сейчас:** (1) любой порог свежести по этой таблице слеп — отрицательный
  возраст всегда «моложе» порога; (2) любой отчёт «за последний час/смену» по `Interaction` считает
  не то окно; (3) гейт живости 08.09 на это не оперся только потому, что сработала ветка GROWING —
  то есть дефект был закрыт удачей, а не проверкой.
- **Чем закрывать (порядок обязателен):** сначала предъявить, ОТКУДА берётся `UpdateTime` —
  из адаптера или из базы; затем сравнить с `RTSData_UserStatus`, где той же беды нет. Правки кода
  до этого не делать: две таблицы пишутся разными путями, и правка «на глаз» замаскирует источник.
- status: **OPEN** (закрытие — только словом оператора)

- [2026-09-08] **PUSH-0831 — ЗАКРЫТ СЛОВОМ ОПЕРАТОРА.** Пуш `f6d5c58 -> 79e3905` (74 файла, 2026-08-31),
  прошедший при объявленном режиме `NO push`, выполнен **по распоряжению оператора** (его слово,
  2026-09-08, в ответ на прямой вопрос `curator-0908`). Нарушением НЕ является: режим `NO push` адресован
  ролям и не связывает оператора. Разбора не будет, виновных нет.
  `CLOSED 2026-09-08 — operator CONFIRM.`
  **Пункт стоял открытым восемь суток и трижды поднимался координатором**, потому что распоряжение было
  дано в чате и не оставило следа на шине: роли видели факт пуша и не видели основания. Цена — три хода
  и один невыполненный долг предыдущей инкарнации куратора.
  **Урок (не норма — сенсор):** распоряжение оператора, меняющее объявленный режим, должно попадать на
  шину строкой в момент исполнения — иначе оно неотличимо от нарушения, а роли обязаны реагировать на
  него как на нарушение. Записывает тот, кто исполняет распоряжение.
  Режим `NO push` на момент записи ОСТАЁТСЯ в силе (вводная оператора этого пробуждения).


### ДОПОЛНЕНИЕ К `PR234-TIME-01` — 2026-09-08, числа второго и третьего замеров (пункт OPEN)
- Отклонение УСТОЙЧИВОЕ, не разовое: возраст свежей строки `RTSData_Interaction` на трёх выборках
  подряд = **−7182 / −7165 / −7200 с** при `RTSData_UserStatus` 6 / 6 / 0 с в тех же выборках.
  Близко к ровным двум часам и держится между прогонами (`234_20260908_162253`, `…_163714`).
- В расчёт свежести не входит — исключено явной строкой `ANOMALY` в приборе `state-4`.
- Гипотеза (локальное время против UTC) НЕ усилена: измерен факт и его устойчивость, не причина.

## PR234-NET-01 — внешнее имя `insightense.com` не разрешается ИЗНУТРИ самой машины 234 · ЗАВЕДЁН 2026-09-08
- **Как найдено:** побочно, при замере живости. Прибор честно взял адрес из `Kestrel`
  (`https://insightense.com:8444`) и получил `000` — соединение не состоялось вовсе; тот же порт по
  петле (`https://127.0.0.1:8444/health`) отвечает `200 Healthy`, негконтроль на нём `404`.
- **Что это значит:** продукт жив, а внешнее имя изнутри машины не резолвится либо не заворачивается.
  Факт о СЕТИ, не о приложении. Причина не установлена, версия не подаётся.
- **Чем это уже стоило:** ложное красное `liveness=False` в замере 15:50 — то есть дефект сети
  выглядел как мёртвый продукт. Прибор с тех пор различает недостижимость и отказ, но сам факт остался.
- **Чем закрывать:** короткий замер DNS + маршрут с машины (`Resolve-DnsName`, hosts, попытка по IP).
  Не начат: отдельного слова на него не давал.
- status: **OPEN** (закрытие — только словом оператора)


## DEV-EOL-01 — норма §35 (BOM+CRLF) против того, как репозиторий хранит скрипты (LF) · ЗАВЕДЁН 2026-09-08
- **Как найдено:** побочно, при §4 над правкой ресинка. `devops-0908c` заметил, что на диске
  `Provision-FreshDb.ps1` идёт с BOM, но без CRLF, и честно НЕ стал чинить это в том же диффе.
- **Замер координатора, который меняет вывод:** в ветке **оба** файла хранятся с LF —
  `v3:db/tools/Provision-FreshDb.ps1` CRLF 0 / LF 292, `v3:db/tools/Restore-All.ps1` CRLF 0 / LF 193,
  BOM у обоих. CRLF, видимый на диске у `Restore-All` (214), — результат нормализации при выдаче
  в рабочую копию.
- **Следовательно противоречие не в файлах, а между нормой и устройством репозитория:** §35 требует
  BOM+CRLF, ветка хранит LF, `git` нормализует при checkout. Значит любая сверка «блоб против диска»
  по этим файлам может законно расходиться, и это надо называть ЗАРАНЕЕ, а не расследовать post factum.
- **Чем закрывать:** решить осознанно — либо `.gitattributes` фиксирует `eol=crlf` для `*.ps1` и
  норма остаётся, либо норма §35 переписывается под «BOM обязателен, EOL нормализуется git'ом».
  Правкой файлов вопрос не решается и в чужие диффы не подмешивается.
- **Чего этот пункт НЕ означает:** ни один сегодняшний прогон им не блокируется; PowerShell 5.1
  разбирает LF нормально — на это опирается сегодняшняя правка.
- status: **OPEN** (закрытие — только словом оператора)


## PR234-ADPT-CFG-01 — адаптер по умолчанию читает `log4net.config` из каталога БОЕВОГО адаптера · ЗАВЕДЁН 2026-09-08
- **Найдено:** `devops-0908c`, попутно при написании промпта на другой дефект. Проверено координатором
  независимо по стору.
- **Факт:** `adapters:RTM.Twilio/appsettings.json:10` и `appsettings.Sample.json:10` содержат
  `"LogConfig": "C:\IceDash\RTM.Twilio\log4net.config"`. `C:\IceDash\` — каталог боевого адаптера,
  запрещённый к изменению; наш экземпляр живёт в `C:\RTMView\RTM.Twilio\`.
- **Чем это опасно, помимо очевидного:** если бы файл в чужом каталоге оказался читаемым, наш адаптер
  молча работал бы на ЧУЖОМ конфиге, и «починка» логирования выглядела бы состоявшейся. Отказ,
  который лечится сам собой не туда, хуже отказа явного.
- **Почему отдельный пункт, а не часть промпта №2:** там предмет «файл не едет в publish», здесь —
  «путь по умолчанию указывает не туда». Разные дефекты, одна правка на один промпт (норма §4).
- **Чем закрывать:** значение по умолчанию должно указывать на каталог рядом с исполняемым файлом
  (или разрешаться относительно него), а не на абсолютный чужой путь. Правку — отдельным промптом,
  после промпта №2, чтобы не мешать две переменные в одной сборке адаптера.
- status: **OPEN** (закрытие — только словом оператора)


### УСЛОВИЕ К `PR234-ADPT-CFG-01` (и к правке №2 промпта `log4net`) — 2026-09-08, coordinator-0908
- **Недостающее звено, названное самим devops-0908c:** sha256 файла
  `C:\RTMView\RTM.Twilio\log4net.config`, лежащего на 234 ПРЯМО СЕЙЧАС, не снят. Пришпилено:
  байты происходят со 140 (sha `1D520F4D…13FA2`, назван в хендофе `devops-0908b`), и живой адаптер
  читает ИМЕННО этот путь (`.measurements/234_20260908_120326_adapter-config.txt:14/:45`).
  Не пришпилено: что содержимое по этому пути сегодня равно эталону.
- **Условие, а не намерение:** снятие sha256 этого файла входит ОБЯЗАТЕЛЬНЫМ пунктом в ближайший
  читающий бокс по 234. Расхождение с `1D520F4D…13FA2` -> правка №2 пересматривается.
- **Почему записано здесь:** условие, живущее только в переписке, исчезает вместе с инкарнацией.
- status: держится до первого читающего замера 234


### ПОПРАВКА К `DEV-EOL-01` — 2026-09-08, coordinator-0908 (формулировка была НЕВЕРНА, предмет остаётся OPEN)
- **Было записано:** «`.gitattributes` у нас не настроен». **Это неверно, и повторяли это двое** —
  devops в отчёте и я в ответе.
- **Измерено:** `v3:.gitattributes` СУЩЕСТВУЕТ и несёт `* text=auto eol=lf`, `*.ps1 text eol=crlf`,
  `*.bat`/`*.cmd eol=crlf`. На ветке `adapters` файла НЕТ.
- **Действительный предмет:** поведение переводов строк РАЗНОЕ ПО ВЕТКАМ. На `v3` норма §35 для
  `.ps1` обеспечена механизмом; на `adapters` всё отдано `core.autocrlf` рабочей машины — что уже
  дало расхождение блоба и рабочей копии в коммите `e01f944` (740 B LF в сторе против 761 B CRLF
  после checkout, содержимое идентично).
- **Отсюда практическое правило до закрытия пункта:** в коммитах, трогающих файлы на `adapters`,
  нормализация называется ЗАРАНЕЕ и приводятся ОБА sha — блоба и рабочей копии.
- **Чем закрывать:** перенести `.gitattributes` на `adapters` (или решить осознанно, что там другое
  правило). Решение оператора; правкой файлов попутно не делается.
- status: **OPEN**


## PR234-ANCHOR-LTR-01 — LTR-половина приёмки якоря не снята и снять её негде · ЗАВЕДЁН 2026-09-19
- **Как вскрылось:** `shell-0919b` перед замером сверил приёмку с корпусом и нашёл, что число «LTR 169 px»
  снято на **RTL**-странице; оговорка автора стоит в самом файле замера `.coord/measure/edge-0919/fit-proto.md`
  и не была прочитана — подпись пошла дальше в приёмку [измерено `shell-0919b`, подтверждено координатором].
- **Действительное состояние:** все четыре строки прототипа сняты на `dir=rtl`, `innerWidth 2560`;
  левый край вылет 169, правый край вылет 47 за окно, средняя колонка 0.
- **Почему LTR не снять:** боевой экран на иврите, LTR-площадки нет; смена локали интерфейса на боевом
  стенде — чужая площадка и решение оператора [не измерено в этом пробуждении: `shell-handoff.md`, 18.09].
- **Решение координатора 19.09:** приёмка якоря переписана на RTL — ОБА края по нулю (правый внесён явно:
  на нём провалилась первая версия прототипа), средняя колонка 0, физических `left`/`right` в диффе нет.
  **LTR остаётся открытой половиной**, названной явно, и закрывается отдельно.
- status: **OPEN** (нужна площадка; закрытие — словом оператора)

## PR234-ENV-LEGACY-RTM-01 — служба legacy `RTM` на 234 работает, хотя числилась остановленной · ЗАВЕДЁН 2026-09-19
- [измерено `devops-0919`, `.measurements/234_20260919_122026_predeploy-state.txt`] `RTM` = `Running`,
  `StartType Automatic`; боевой `RTM.Twilio` = `Stopped`.
- В унаследованной картине и в шапке передачи `RTM` значилась остановленной [не измерено: хендоф, 18.09].
- Кем и когда поднята — **не установлено**, толкования не подаются.
- Трогать запрещено; на путь выката (`Update-RTMView`) не влияет. **Перед приёмкой выката состояние
  снимается повторно**, чтобы чужое изменение не было приписано нашему выкату.
- status: **OPEN**

## PR234-HEALTH-503-01 — `/health` отвечает 503 по HTTP при 200 по HTTPS · ЗАВЕДЁН 2026-09-19
- [измерено `devops-0919`, тот же отчёт] `https://platform.insightense.com:8444/health` -> **200 Healthy**,
  петля `https://127.0.0.1:8444` -> 200 Healthy, `http://127.0.0.1:5000/health` -> **503 Unhealthy**,
  `http://platform.insightense.com:5000/health` -> 307.
- **Это ответ приложения, а не недостижимость** — то есть что-то внутри считает себя нездоровым, и это
  противоречит соседнему зелёному. Что именно — не измерено.
- Живость парой при этом зелёная: негконтроль на ответившем адресе 404, пайп `rtmpipe_v3` обслуживается.
- **Чем закрывать:** снять ТЕЛО ответа `/health` на 5000 — отдельным читающим заходом ПОСЛЕ выката,
  чтобы не мешать два вопроса в одном прогоне.
- status: **OPEN**

### ОТМЕТКА К условию по `log4net` (правка №2, условие от 08.09) — ВЫПОЛНЕНО ПО ЗАМЕРУ 2026-09-19
[измерено `devops-0919`] `C:\RTMView\RTM.Twilio\log4net.config` sha256 `1D520F4D…13FA2`, match TRUE;
`LogConfig` в конфиге адаптера указывает на наш каталог, не на боевой `C:\IceDash`. Правка №2
пересмотра не требует. Формальное закрытие пункта — словом оператора.


## PUSH-BARRIER-0919 — 43 непушенных, `origin` стоит с 14.09 · РЕШЕНИЕ ОПЕРАТОРА 2026-09-19: ЖДЁМ УСЛОВИЙ КООРДИНАТОРА

- **Факт [измерено куратором 2026-09-19]:** `v3 = 29b1681`, `origin/v3 = ed3e292`, непушенных **43**.
  За один сегодняшний день ветка выросла на 14 коммитов. Обход трекаемой зоны: 342 тела, 0 расхождений —
  то есть всё сделанное сохранено В ВЕТКЕ, но ветка целиком живёт на одном диске.
- **Чем этот риск отличается от того, что мы чинили весь день.** Незакоммиченное умирает от одной
  операции и восстановлению не подлежит; непушенное переживает всё, кроме утраты диска. Это не та же
  болезнь в большем масштабе, а другая: цена ниже, но и лечение не наше.
- **Позиция линии координатора** (`coordinator-0919`, унаследована `coordinator-0919b`): барьер держится
  осознанно. Основание — разведение стоимости сторон: **пуш дорог и требует свежего замера, удержание
  барьера не требует НИ ОДНОГО нового факта о мире, потому что сохраняет уже существующее состояние.**
  Н-11б запрещает опираться на унаследованное в дорогом решении и ничего не требует, чтобы НЕ делать
  необратимого.
- **Два условия открытия, названные им предъявляемыми:** живые половины предметов сняты ЧИСЛАМИ, и
  слово оператора на пуш.
- **РЕШЕНИЕ ОПЕРАТОРА 2026-09-19: ждём.** Зафиксировано куратором со слов оператора. Пункт не закрыт и
  не отложен бессрочно — он ОЖИДАЕТ СОБЫТИЯ.
- **Условие пробуждения:** выполнение обоих условий координатора, ЛИБО рост до 60 непушенных, ЛИБО
  любая работа на боевом 234, которую придётся откатывать по ветке. Любое из трёх — поднимаю оператору
  первым предметом. Ответственный за подъём — куратор; напоминание живёт здесь, а не в чьей-либо памяти.
- **Чего решение НЕ означает:** оно не запрещает пуш по отдельному слову оператора в любой момент и не
  отменяет барьер §37 как процедуру. Оно означает только, что инициатива поднятия вопроса сейчас не за мной.
- status: **WAITING-ON-EVENT** (закрытие — только словом оператора)
