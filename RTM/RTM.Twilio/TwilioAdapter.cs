    ...
    public class TwilioAdapter : BackgroundService
    {
        ...
        public TwilioAdapter(ILogger<TwilioAdapter> logger, IConfiguration configuration)
        {
            ...
            try
            {
                ...
                string rtmURL = configuration.GetValue<string>("RTM:RTM_URL");

                RTMAdapter.ServerConnectEvent += RTMAdapter_ServerConnectEvent;
                _ = RTMAdapter.connect(rtmURL);
            }
            catch (Exception ex)
            {
        ...
        private async Task setInteractionAsync(string name, JToken data, DateTime timestamp)
        {
            try { 
                ...
                // Cencel Interaction
                if (name == "ReservationCompleted" || name == "TaskCanceled" || 
                   ...
                   (name == "ReservationTimeout" && transferMode != string.Empty))    // || name == "ReservationCanceled" || name == "ReservationRejected" || name == "ReservationTimeout") // || newState == "ExternalDisconnect")
                {
                    if (Interactions.TryGetValue(interactionId, out interaction))
                    {
                        interaction.TimeStamp = eventTimeSatamp;

                        if (name == "ReservationCompleted" || name == "ReservationCanceled" || name == "ReservationRejected" || name == "ReservationTimeout" || name == "TaskTransferCanceled")
                        {
                        ...
                        else if (name == "TaskCanceled" || (name == "TaskUpdated" && state == "completed"))// || name == "TaskQueueMoved")
                        {
        ...
        private async Task workerUpdateAsync(string eventName, JToken data, DateTime timestamp)
        {
            try
            {
                ...
                {
                    AsyncLogger.Info($"eventTimeStampUtc={eventTimeStampUtc} <= agent.TimeStamp=agent.TimeStamp");
                    return;
                }
                else
                {
                ...
                if (eventName == "WorkerActivityUpdate")
                {
                    ...
                    else
                    {
                        ...
                        await userStatusChangedAsync(workerName,
                           ...
                           statusGroup, timestamp, "", false, DateTime.MinValue);

                        agent.StatusGroup = statusGroup;
                    }

                    agent.UserId = workerName;
                    agent.StatusChanged = timestamp;
