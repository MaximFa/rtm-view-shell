...
> ⚠ **ИЗМЕНЕНО решением оператора 2026-09-19** — см. раздел «AMENDMENT 2026-09-19» в конце файла.
...
# AMENDMENT 2026-09-19 — ЧЕТЫРЕ ОБЯЗАТЕЛЬНЫХ БЛОКА CC-ПРОМПТА РАЗДЕЛЕНЫ. Решение оператора.

[со слов оператора: 2026-09-19] Изменяет §0.5, §0.6a, §42.5, §42.6 в части, названной ниже.
Всё, что здесь не названо, остаётся в прежней силе — в том числе §0.6b (binding) и §0.6 (post-commit).

**Почему.** Требование «четыре блока в каждом CC-промпте, MANDATORY, NO EXCEPTIONS» корпусом
не исполняется, и это измерено, а не замечено:
```
[измерено 2026-09-19, обход 555 промптов ветки]
месяц   всего   pre-commit-check   commit.lock
06       432    300 (69%)          235 (54%)
07        18     15 (83%)           17 (94%)
08        90     49 (54%)           65 (72%)
09        15      1 ( 6%)            3 (20%)
[измерено] .coord/journal.md: записей 08 — 8 при 69 коммитах · 09 — 9 при 101 коммите
           то есть БОЛЬШЕ 90% коммитов месяца не оставили следа; осыпание началось в АВГУСТЕ
```
Норма со словом MANDATORY, которую корпус не исполняет, обесценивает соседние: роль перестаёт
различать, какое из требований настоящее. Снять целиком нельзя — две трети корпуса её исполняли.
Поэтому блоки **разделены по тому, что именно каждый стережёт**, а не приняты и не отменены пачкой.

## Что теперь обязательно, а что нет

**1. `commit.lock` — ОСТАЁТСЯ ОБЯЗАТЕЛЬНЫМ, им гейтим.**
Единственный из четырёх, чьё отсутствие НЕОБРАТИМО и НЕЗАМЕТНО: при гонке двух коммитов один
пропадает молча, и восстановить его нечем — он никогда не существовал. Заметить постфактум
невозможно по построению.

**2. `pre-commit-check` — СУЖЕН до записи через монтирование.**
Он стережёт обрезанный файл при частичной записи через сетевой маунт (PD-005/006/007).
Там, где сессия пишет на машину напрямую, он не стережёт ничего и платится впустую.
Обязателен для записи через маунт; в остальных случаях не предъявляется.

**3. `journal` — ОБЯЗАННОСТЬ ОТМЕНЕНА. Вместо неё — авторство в сообщении коммита.**
Журнал дублирует историю git: что закоммичено, когда и с каким сообщением, уже лежит в ветке
и оттуда не пропадает. Обязанность переписывать это руками в незакоммиченный файл осыпалась
не от расхлябанности, а потому что бессмысленна.
**Но в git действительно нет одного: КАКАЯ РОЛЬ И КАКАЯ СЕССИЯ сделала коммит.** Это и есть
настоящая дыра, и закрывается она строкой в сообщении коммита, а не пунктом в промпте:
след тогда лежит в объекте, неубиваемо, а не в файле, который сам живёт в одном экземпляре.
`.coord/journal.md` не удаляется и не чистится — он остаётся свидетелем июня-июля.

**4. `session-coord` в списке обязательных блоков промпта — СНЯТ.**
Предписывает читать то, что роль и так читает при подъёме по своему иниту.

## Отрицательная половина этого решения — чем куратор признаёт себя неправым
- **Гонка коммитов при живом `commit.lock`** -> замка мало, сужение было ошибкой.
- **Авторство в сообщении коммита не приживается так же, как не прижился журнал** -> дело не
  в дублировании, а в том, что роли не держат дисциплину следа вообще; тогда прав автооткат
  нормы (кандидат `coordinator-0919b`), а не это разделение.
Оба признака проверяются обходом ветки, а не ощущением.

## Что при этом НЕ меняется
Запрет Edit-инструмента (§0). `fsync` после записи. §0.6 post-commit. §0.6b binding —
практикой ЖИВ и измерен: 31 блок в `cc/shell.md`, 16 в `cc/backend.md`, 113 заголовков
в `cc/devops.md`. Барьер пуша. Кворум приёмки.
