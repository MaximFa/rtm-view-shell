-- =====================================================================
-- patch_origin_v2.sql  —  idempotent server-side patch to origin/v2 target
-- =====================================================================
-- Brings a production rtmviewdb to the origin/v2 state in ONE psql run:
--   * Metrics D3a : 12 catalogue columns on public."RTSGrid_Metric"
--                   + EF history registration + catalogue backfill (~198 rows)
--   * DayTrend    : RTSData_SetUserStatus = single 15-param PROCEDURE that
--                   writes RTSData_UserStatusLog (DayTrend agent history)
--
-- Safe to re-run (ADD COLUMN IF NOT EXISTS / ON CONFLICT / value-matched
-- UPDATEs / DROP+CREATE PROCEDURE).  Aborts on first error (ON_ERROR_STOP).
--
-- RUN AS the owner of public."RTSGrid_Metric" (ccdashboard_user) or a
-- superuser, e.g.:
--   psql -h localhost -U ccdashboard_user -d rtmviewdb -f patch_origin_v2.sql
--
-- WHY this is needed: RTSGrid_Metric belongs to BackendEmulationDbContext,
-- whose EF migrations are applied by Shell ONLY in Development/Testing
-- (DatabaseInitializer gates them) — so the 12 catalogue columns are never
-- created on a production database by the normal Shell `migrate`.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- SECTION 1 — Metrics D3a: catalogue columns + EF history (idempotent)
-- ---------------------------------------------------------------------
\echo '== Section 1: catalogue columns + EF history =='
BEGIN;
...
-- Register the EF migration so a future `dotnet ef` sees it as applied.
...
-- ---------------------------------------------------------------------
-- SECTION 2 — Metrics D3a: catalogue backfill
--   (verbatim from db/migrations/20260606_003_catalog_backfill.sql;
--    has its own BEGIN/COMMIT; idempotent UPDATEs by MetricId)
-- ---------------------------------------------------------------------
\echo '== Section 2: catalogue backfill =='
...
-- ---------------------------------------------------------------------
-- SECTION 3 — DayTrend: RTSData_SetUserStatus (15-param PROCEDURE + log)
--   (verbatim from db/migrations/20260606_002_fix_userstatuslog_write.sql)
-- ---------------------------------------------------------------------
\echo '== Section 3: RTSData_SetUserStatus procedure =='
...
\echo '-- (a) catalogue columns present (expect 12):'
SELECT count(*) AS catalogue_columns
...
\echo '-- (b) backfill coverage: total metrics, catalogued (DisplayName not null), uncatalogued:'
SELECT
...
\echo '-- (b2) metrics still NULL DisplayName (should be only known non-catalogued, if any):'
SELECT "MetricId" FROM public."RTSGrid_Metric" WHERE "DisplayName" IS NULL ORDER BY "MetricId";

\echo '-- (c) RTSData_SetUserStatus: expect 1 row, prokind=p, pronargs=15, has_log=t:'
SELECT proname,
       prokind                                   AS kind_expect_p,
       pronargs                                  AS args_expect_15,
       (prosrc LIKE '%RTSData_UserStatusLog%')   AS has_log_write
FROM pg_proc WHERE proname = 'RTSData_SetUserStatus';

\echo '== patch_origin_v2.sql complete =='
