...
body.push(new Paragraph({ alignment:AlignmentType.CENTER, spacing:{after:40},
  children:[new TextRun({text:"Edition EN · 2026-06-10 · RTM View Shell (Twilio / any-ACD)", size:22, color:"595959"})] }));
body.push(new Paragraph({ alignment:AlignmentType.CENTER, spacing:{before:400},
  children:[new TextRun({text:"All schema facts in this guide are verified against db/schema.sql and the RTM Twilio configuration.",
...
body.push(callout('note',"The sample tenant used throughout Part 2", [
  ...
  [{t:"TenantId",code:true,b:false}," = ",{t:"11111111-1111-1111-1111-111111111111",code:true}," · all examples filter ",{t:"OnDate = '2026-06-09'",code:true}," (yesterday)."],
...
body.push(...nums([
  [{t:"Connect",b:true}," to the RTM PostgreSQL database (PostgreSQL 18; PostgreSQL 17 on some servers). All reporting tables are in schema ",{t:"public",code:true},"; identifiers are quoted and case-sensitive, so always quote them."],
  ...
  [{t:"Always filter on two things",b:true},": ",{t:"\"TenantId\"",code:true}," and ",{t:"\"OnDate\"",code:true},
   ". OnDate is a ",{t:"string",b:true}," date key (varchar), not a timestamp — compare it as text: ",{t:"\"OnDate\" = '2026-06-09'",code:true},"."],
...
body.push(code([
  ...
  '  ON qc."TenantId" = i."TenantId"',
  ' AND qc."QueueId"  = i."Workgroup"',
  " AND qc.\"ClassificationId\" = 'ALL'",
  'JOIN "NGC_BusinessUnit" bu',
  '  ON bu."TenantId" = qc."TenantId"',
  ' AND bu."BusinessUnitId" = qc."BusinessUnitId"',
  "WHERE i.\"TenantId\" = '11111111-1111-1111-1111-111111111111'",
...
body.push(P([{t:"Units are called out per column. ",b:true},
  ...
  ", and all NGC joins use ", {t:"string external keys, never the uuid primary keys",b:true},"."]));

body.push(H3("RTSData_Interaction  — call fact (grain: one interaction segment)"));
body.push(dict([
  ["TenantId","uuid","FK","Tenant. Filter and join on this in every query."],
  ["InteractionId","varchar(50)","PK","Interaction id. Part of the primary key with Segment + OnDate + ServerId."],
  ["Segment","integer","PK","Segment number. One interaction can have several segments (e.g. after a transfer). Count DISTINCT InteractionId to avoid double-counting."],
  ["OnDate","varchar(50)","PK","Business date as a STRING (e.g. '2026-06-09'). The day key for all daily reporting."],
  ["ServerId","varchar(50)","PK","Source server / site id."],
  ["Workgroup","varchar(100)","FK","Queue external id. Join to NGC_Queues.ExternalId / NGC_BusinessUnitQueueClassification.QueueId."],
  ["UserId","varchar(50)","FK","Agent external id that handled the segment."],
  ["ClassificationCode","text","","Optional call classification / disposition code."],
  ["InteractionType","varchar(50)","","'Call', 'Callback', 'Chat'."],
  ["CallType","varchar(50)","","e.g. 'External' / 'Internal'."],
  ["Direction","varchar(50)","","'Incoming' / 'Outgoing'."],
  ["IsAnswered","boolean","","True if the segment was answered by an agent."],
  ["IsInQueue","boolean","","True while waiting in queue (not yet answered)."],
  ["IsTalk","boolean","","True while the agent is actively talking (in-progress flag)."],
  ["IsAbandoned","boolean","","True if the caller left the queue before being answered."],
  ["IsTransferred","boolean","","True if the segment ended in a transfer."],
  ["IsMessaging","boolean","","True for messaging interactions."],
  ["IsCallbackRequest","boolean","","True if the interaction is a callback request."],
  ["TimeInQueue","integer","","Queue wait time in SECONDS."],
  ["TalkTime","integer","","Talk time in SECONDS."],
  ["InQueueDateTime","timestamptz","","When the interaction entered the queue (UTC)."],
  ["AnsweredDateTime","timestamptz","","When the interaction was answered (UTC). Unanswered rows carry a sentinel far-past date."],
  ["UpdateTime","timestamptz","","Last update of the row (UTC)."],
  ["LastUserId","varchar(50)","","Last agent on the interaction (after transfers)."],
  ["LastWorkgroup","varchar(100)","","Last queue on the interaction."],
  ["RemoteAddress","varchar(50)","","Caller number / remote address."],
  ["TimeZone","varchar(10)","","Site time zone of OnDate."],
  ["CustomCallData, CustomCallData1..20","text","","Tenant-defined extension fields (disposition, campaign, etc.)."]
]));
body.push(SP());
...
body.push(dict([
  ...
  ["UserId","varchar(100)","PK/FK","Agent external id. Join to NGC_UserAgentgroup.UserId."],
  ["StatusId","varchar(100)","PK","Status identifier."],
  ["ServerId","varchar(50)","PK","Source server / site."],
  ["OnDate","varchar(50)","PK","Business date STRING."],
  ["StatusName","varchar(100)","","Human status name (the state, e.g. 'Talk', 'Hold', 'Wrap Up', 'Lunch')."],
  ["StatusGroup","varchar(100)","","Denormalised group of this status (AVAILABLE / ONPHONE / BREAK / PAPERWORK / TRAINING / SIGNOFF / UNAVAILABLE)."],
  ["TotalDuration","integer","","Cumulative time in this status today, in SECONDS."],
  ["MaxDuraction","integer","","Longest single stay in this status today, SECONDS. NOTE: the column name is mis-spelled in the database (“MaxDuraction”) — quote it exactly."],
  ["TotalCount","integer","","Number of times the agent entered this status today."],
  ["DisplayName","varchar(100)","","Agent display name."],
  ["UpdateTime","timestamptz","","Last update (UTC)."],
  ["TimeZone","varchar(10)","","Site time zone."]
]));
body.push(SP());
...
body.push(dict([
  ["Id","integer","PK","Surrogate identity key."],
  ["TenantId","uuid","FK","Tenant."],
  ["UserId","varchar(100)","FK","Agent external id."],
  ["StatusId","varchar(100)","","Status identifier (the state for this interval)."],
  ["ServerId","varchar(50)","","Source server / site."],
  ["OnDate","varchar(50)","","Business date STRING."],
  ["StartTime","timestamptz","","Interval start (UTC). Use for point-in-time reconstruction."],
  ["EndTime","timestamptz","","Interval end (UTC). NULL / open for the current ongoing status."],
  ["Duration","integer","","Interval length in MILLISECONDS (see trap above)."],
  ["StatusGroup","varchar(50)","","Group of this status, denormalised onto the interval."],
  ["UpdateTime","timestamptz","","Last update (UTC)."],
  ["TimeZone","varchar(10)","","Site time zone."]
]));
body.push(SP());
...
body.push(dict([
  ["BusinessUnitId","integer","PK","BU id (identity). Join target for both roll-up paths."],
  ["TenantId","uuid","FK","Tenant."],
  ["BusinessUnitName","varchar(100)","","Display name (e.g. 'Sales')."],
  ["Description","text","","Free text."],
  ["SiteId","varchar(50)","FK","Site (NGC_Site.SiteId)."],
  ["CreatedDatetime / CreatedBy","timestamptz / varchar","","Audit."]
]));
body.push(H3("NGC_Queues"));
body.push(dict([
  ["Id","uuid","PK","Surrogate key (do NOT join on this)."],
  ["TenantId","uuid","FK","Tenant."],
  ["ExternalId","varchar(100)","KEY","Queue external id — equals RTSData_Interaction.Workgroup."],
  ["Name","varchar(200)","","Display name."],
  ["IsActive","boolean","","Active flag."]
]));
body.push(H3("NGC_BusinessUnitQueueClassification  (queue → BU map)"));
body.push(dict([
  ["BusinessUnitId","integer","FK","Target BU."],
  ["QueueId","varchar(100)","KEY","Queue external id (= Interaction.Workgroup)."],
  ["TenantId","uuid","FK","Tenant."],
  ["ClassificationId","varchar(100)","","Must be 'ALL' for a standard “all calls of this queue” mapping. Always include qc.\"ClassificationId\" = 'ALL' in the join."]
]));
body.push(H3("NGC_Supergroup"));
body.push(dict([
  ["SupergroupId","integer","PK","Super Group id (identity)."],
  ["TenantId","uuid","FK","Tenant."],
  ["SupergroupName","varchar(200)","","Display name (e.g. 'Sales Team A')."],
  ["Description","varchar(500)","","Free text."]
]));
body.push(H3("NGC_AgentGroups"));
body.push(dict([
  ["Id","uuid","PK","Surrogate key (do NOT join on this)."],
  ["TenantId","uuid","FK","Tenant."],
  ["ExternalId","varchar(100)","KEY","Agent-group external id (= SupergroupAgentgroup.AgentgroupId / UserAgentgroup.AgentgroupId)."],
  ["Name","varchar(200)","","Display name."],
  ["IsActive","boolean","","Active flag."]
]));
...
body.push(dict([
  ...
  ["SupergroupId","integer","FK","Super Group."],
  ["AgentgroupId","varchar(100)","KEY","Agent-group external id."],
  ["TenantId","uuid","FK","Tenant."]
]));
body.push(H3("NGC_BusinessUnitSupergroup  (super-group → BU map)"));
body.push(dict([
  ["BusinessUnitId","integer","FK","Business Unit."],
  ["SupergroupId","integer","FK","Super Group."],
  ["TenantId","uuid","FK","Tenant."]
]));
...
body.push(dict([
  ["Id","integer","PK","Surrogate identity."],
  ["UserId","varchar(100)","KEY","Agent external id (= RTSData_UserStatus.UserId)."],
  ["AgentgroupId","varchar(100)","KEY","Agent-group external id."],
  ["TenantId","uuid","FK","Tenant."]
]));
body.push(H3("NGC_Site"));
body.push(dict([
  ["SiteId","varchar(50)","PK","Site id (e.g. 'IL')."],
  ["TenantId","uuid","FK","Tenant."],
  ["SiteName","varchar(200)","","Display name."],
  ["TimeZone","varchar(10)","","Site time zone."],
  ["ClearTime","varchar(5)","","Daily midnight-clear time (HH:MM)."]
]));
...
body.push(P([{t:"Path A — calls: Workgroup → Queue → BU.",b:true},
  " ClassificationId = 'ALL'. TenantId on every join."]));
body.push(code([
  'FROM "RTSData_Interaction" i',
  'JOIN "NGC_BusinessUnitQueueClassification" qc',
  '       ON qc."TenantId" = i."TenantId"',
...
body.push(code([
  ...
  'JOIN "NGC_SupergroupAgentgroup" sa ON sa."TenantId"=ua."TenantId" AND sa."AgentgroupId"=ua."AgentgroupId"',
  'JOIN "NGC_Supergroup"           sg ON sg."TenantId"=sa."TenantId" AND sg."SupergroupId"=sa."SupergroupId"',
  'JOIN "NGC_BusinessUnitSupergroup" bs ON bs."TenantId"=sg."TenantId" AND bs."SupergroupId"=sg."SupergroupId"',
  'JOIN "NGC_BusinessUnit"         bu ON bu."TenantId"=bs."TenantId" AND bu."BusinessUnitId"=bs."BusinessUnitId"'
]));
...
body.push(tbl(["Gotcha","What it means"],
  [["Milliseconds vs seconds","UserStatusLog.Duration = MILLISECONDS; UserStatus.TotalDuration/MaxDuraction and Interaction.TimeInQueue/TalkTime = SECONDS. Divide the log by 1000 before mixing."],
   ...
   ["OnDate is a string","varchar, not a date/timestamp. Compare as text: \"OnDate\" = '2026-06-09'."],
   ...
   ["String keys, not PKs","All NGC joins use external string ids (UserId, AgentgroupId, QueueId) and integer BU/SG ids — never the uuid surrogate keys."],
   ...
   ["TenantId everywhere","Every table is multi-tenant. Put TenantId in every WHERE and every JOIN, or you leak across tenants."]],
...
body.push(tbl(["Field","CALL-1001","CALL-1002","CALL-1003","CALL-1004"],
  [["Workgroup","SALES_VOICE","SALES_VOICE","SALES_CB","SUP_VOICE"],
   ...
   ["IsAnswered","true","false","false","false"],
   ["IsAbandoned","false","true","true","true"],
...
body.push(code([
  ...
  'FROM "RTSData_Interaction" i',
  'JOIN "NGC_BusinessUnitQueueClassification" qc',
  '  ON qc."TenantId"=i."TenantId" AND qc."QueueId"=i."Workgroup"',
  "  AND qc.\"ClassificationId\"='ALL'",
  'JOIN "NGC_BusinessUnit" bu',
  '  ON bu."TenantId"=qc."TenantId" AND bu."BusinessUnitId"=qc."BusinessUnitId"',
  "WHERE i.\"TenantId\" = '11111111-1111-1111-1111-111111111111'",
  "  AND i.\"OnDate\"   = '2026-06-09'",
  "  AND bu.\"BusinessUnitName\" = 'Sales'",
...
body.push(code([
  ...
  'JOIN "NGC_SupergroupAgentgroup" sa ON sa."TenantId"=ua."TenantId" AND sa."AgentgroupId"=ua."AgentgroupId"',
  'JOIN "NGC_Supergroup"           sg ON sg."TenantId"=sa."TenantId" AND sg."SupergroupId"=sa."SupergroupId"',
  "WHERE us.\"TenantId\" = '11111111-1111-1111-1111-111111111111'",
...
body.push(code([
  ...
  "WHERE l.\"TenantId\" = '11111111-1111-1111-1111-111111111111'",
  "  AND l.\"OnDate\"   = '2026-06-09'",
...
body.push(...bullets([
  ...
  [{t:"Joining on uuid PKs",b:true}," — join NGC tables on the string external ids (ExternalId / AgentgroupId / UserId / QueueId) and integer BU/SG ids, not on NGC_Queues.Id / NGC_AgentGroups.Id."],
  [{t:"Counting segments as calls",b:true}," — use count(DISTINCT InteractionId) for call volumes."],
  [{t:"Missing TenantId",b:true}," — every WHERE and JOIN must carry TenantId."],
...
body.push(P([{t:"Verification note: ",b:true,i:true},
  {t:"every column, type and unit in this guide was checked against db/schema.sql and the RTM Twilio configuration on 2026-06-10.",i:true}]));
...
body.push(code(["-- QueueNumAbandonedCalls   (Function: InteractionsCount)", "SELECT count(DISTINCT i.\"InteractionId\") AS abandoned_calls", "FROM \"RTSData_Interaction\" i", "WHERE i.\"TenantId\" = '...'            -- multi-tenant edition", "  AND i.\"OnDate\"   = '2026-06-09'", "  AND i.\"InteractionType\"   = 'Call'", "  AND i.\"CallType\"          = 'External'", "  AND i.\"Direction\"         = 'Incoming'", "  AND i.\"IsAbandoned\"       = true", "  AND i.\"IsCallbackRequest\" = false;   -- !IsCallbackRequest", "-- scope to a BU with the queue->BU join from section 2.2"]));
...
body.push(code(["-- MonAgentBreakDuration   (Function: TotalStatusGroupDuration)", "-- Metric Parameter 'BREAK' = a StatusGroup value", "SELECT us.\"UserId\", sum(us.\"TotalDuration\") AS break_seconds   -- SECONDS", "FROM \"RTSData_UserStatus\" us", "WHERE us.\"TenantId\" = '...'", "  AND us.\"OnDate\"   = '2026-06-09'", "  AND us.\"StatusGroup\" = 'BREAK'", "GROUP BY us.\"UserId\";"]));
...
body.push(code(["-- MonAgentNumberOfInboundCalls15sec   (Function: InteractionsCount)", "SELECT count(DISTINCT i.\"InteractionId\") AS short_inbound", "FROM \"RTSData_Interaction\" i", "WHERE i.\"TenantId\" = '...'", "  AND i.\"OnDate\"   = '2026-06-09'", "  AND i.\"CallType\"  = 'External'", "  AND i.\"Direction\" = 'Incoming'", "  AND i.\"TalkTime\"  < 15                       -- seconds", "  AND i.\"InteractionType\" IN ('Call','Callback');"]));
...
const doc = new Document({
  ...
  sections: [{
    ...
    headers: { default: new Header({ children:[ new Paragraph({
      ...
      children:[ LOGOSM(), new TextRun({ text:"   RTM View Shell Data Connector — Unified Reporting Guide", size:16, color:"808080" }) ] }) ] }) },
...
Packer.toBuffer(doc).then(buf=>{ fs.writeFileSync("/tmp/build/RTM_Unified_Reporting_Guide_EN.docx", buf);
