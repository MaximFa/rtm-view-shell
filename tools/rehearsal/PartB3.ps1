...
$env:PGCLIENTENCODING = "UTF8"   # data contains Hebrew; console WIN1252 would kill the verify
...
Write-Host ""
Write-Host "===== PRE-EXISTING integrity IN THE SOURCE (before we touch anything) =====" -ForegroundColor Yellow
Write-Host "    separates 'arrived broken from 234' from 'broken by our reload'"
$preq = @'
SELECT 'Cell->Row (source)' k, count(*) n FROM "RTSGrid_Cell" c
  LEFT JOIN "RTSGrid_Row" p ON c."RowId"=p."RowId" WHERE p."RowId" IS NULL
UNION ALL SELECT 'Cell->Column (source)', count(*) FROM "RTSGrid_Cell" c
  LEFT JOIN "RTSGrid_Column" p ON c."ColumnId"=p."ColumnId" WHERE p."ColumnId" IS NULL
UNION ALL SELECT 'BUQueueClass->Queues.Ext (source)', count(*) FROM "NGC_BusinessUnitQueueClassification" c
  LEFT JOIN "NGC_Queues" p ON c."QueueId"=p."ExternalId" WHERE c."QueueId" IS NOT NULL AND p."ExternalId" IS NULL
UNION ALL SELECT 'SGAG->AgentGroups.Ext (source)', count(*) FROM "NGC_SupergroupAgentgroup" c
  LEFT JOIN "NGC_AgentGroups" p ON c."AgentgroupId"=p."ExternalId" WHERE c."AgentgroupId" IS NOT NULL AND p."ExternalId" IS NULL
UNION ALL SELECT 'UserAgentgroup->AgentGroups.Ext (source)', count(*) FROM "NGC_UserAgentgroup" c
  LEFT JOIN "NGC_AgentGroups" p ON c."AgentgroupId"=p."ExternalId" WHERE c."AgentgroupId" IS NOT NULL AND p."ExternalId" IS NULL
ORDER BY 1;
'@
$pf = Join-Path $env:TEMP "b_pre.sql"
[System.IO.File]::WriteAllText($pf, $preq, (New-Object System.Text.UTF8Encoding($false)))
& "$pg\psql.exe" -U postgres -d $src -f $pf
Write-Host ""
Write-Host "===== RELOAD =====" -ForegroundColor Yellow
$colList = '"Id","TenantId","ExternalId","Name","IsActive"'
foreach ($tn in $tables) {
    if ($tn -in @("NGC_Queues","NGC_AgentGroups")) {
        # drifted: explicit column list, staging -> temp text file -> target (a pipe via cmd proved unreliable)
        $tmp = Join-Path $env:TEMP "b_data_$tn.txt"
        if (Test-Path $tmp) { Remove-Item $tmp -Force }
        $o = Join-Path $env:TEMP "b_out_$tn.sql"; $i = Join-Path $env:TEMP "b_in_$tn.sql"
        $tmpEsc = $tmp -replace '\\','/'
        [System.IO.File]::WriteAllText($o, "\copy (SELECT $colList FROM public.""$tn"") TO '$tmpEsc'", (New-Object System.Text.UTF8Encoding($false)))
        [System.IO.File]::WriteAllText($i, "\copy public.""$tn"" ($colList) FROM '$tmpEsc'", (New-Object System.Text.UTF8Encoding($false)))
        $o1 = & "$pg\psql.exe" -U postgres -d $src -f $o 2>&1
        $o2 = & "$pg\psql.exe" -U postgres -d $db  -f $i 2>&1
        $bad = @($o1; $o2) | Select-String -Pattern "ERROR|error:"
        ...
        else { Write-Host ("  {0,-40} ok (column-list copy, {1} rows, CreatedDatetime dropped)" -f $tn, $rows) -ForegroundColor Green }
    } else {
