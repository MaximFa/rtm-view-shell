...
Write-Host ""
Write-Host ("tables with drift: {0}" -f $(if ($drift.Count) { $drift -join ", " } else { "none" })) -ForegroundColor $(if ($drift.Count -gt 2) { "Red" } else { "Cyan" })
if ($drift.Count -gt 2) { Write-Host "STOP: drift beyond the two known tables - report to coordinator" -ForegroundColor Red; & "$pg\dropdb.exe" -U postgres --if-exists $src; $env:PGPASSWORD=""; exit 1 }
