# CC task — CREATE role-incident (Incident Responder) + seed the incident ledger
> §4-PASS coordinator-0612 2026-06-20T12:12:27Z — but DO NOT RUN until curator discipline-pass per the role-creation norm (role-coordinator §D). Commit `docs:`. NO push.
> Operator directive 2026-06-20: create an Incident Responder role on the coordinator knowledge base but incident-tuned, via the role-creation norm. Cross-cutting (like Security), diagnose+triage, NO code territory, routes fixes to owners. Cold-start from technical artifacts + incident anchor.

## Mandatory — read before starting
Read: .coord/protocols/role-skill-standard.md + role-skill-TEMPLATE.md (house format)
Read: .coord/incidents context if any; CLAUDE.md §34 (RTM Relay/SignalR) + §2 (stack: Redis backplane/Memurai)

## INIT + discipline
- §0.2 integrity; branch v2-backend; §0.5 object-store. §0.3 Python+fsync. Binding PREAMBLE -> .coord/cc/coordinator.md. commit.lock around commit. NO push (§37).

## THE WORK
1. `mkdir -p .claude/skills/role-incident` + `docs/incidents` (os.makedirs exist_ok).
2. Write `.claude/skills/role-incident/role-incident.md` VERBATIM from the SKILL PAYLOAD (do NOT reformat — the §A invariants + source-pins are deliberate).
3. Write `docs/incidents/incidents.md` VERBATIM from the LEDGER PAYLOAD (seeded with INC-2026.06.20-001).
4. VERIFY (object-store): role-incident.md frontmatter (role: incident); §A has the 10 invariants + "ENUMERATE ALL"; §B has the INC-001 worked-lesson; incidents.md has INC-2026.06.20-001 row. tail proper end.

### SKILL PAYLOAD -> .claude/skills/role-incident/role-incident.md
--- BEGIN role-incident.md ---
---
role: incident
...
--- END role-incident.md ---

### LEDGER PAYLOAD -> docs/incidents/incidents.md
--- BEGIN incidents.md ---
...
`docs: create role-incident (Incident Responder) — cross-cutting diagnose+triage, ledger-keeping, known-detection; seed docs/incidents/incidents.md INC-2026.06.20-001 (Redis backplane down)`
then §0.6 post-commit + §0.7 re-sync + sync.

## REPORT -> binding cc/coordinator.md RESULT + inbox/coordinator.md. NO push.
