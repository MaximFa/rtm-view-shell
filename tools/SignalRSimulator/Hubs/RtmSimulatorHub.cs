...
public class RtmSimulatorHub : Hub
{
    ...
    // Agent state names (from tenant_agent_states)
    private static readonly string[] AgentStates = ["AVAILABLE", "ONPHONE", "BREAK", "PAPERWORK", "TRAINING"];
    // Agent state group names (from tenant_agent_state_groups)
    private static readonly string[] AgentStateGroups = ["Available", "On Phone", "Break", "Paperwork", "Training"];
    // Telephony states
    private static readonly string[] TelStates = ["Idle", "Ringing", "Talking", "Hold", "Wrap"];
    // Interaction types
    private static readonly string[] InteractionTypes = ["", "Voice", "Chat", "Email", "Callback"];
    // Campaign names
    private static readonly string[] Campaigns = ["", "Sales Q2", "Support", "Retention", "Survey"];
    // Agent names
    ...
    // --- Agent grid generation with ALL metrics from RTSGrid_Metric ---
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                var agentRows = agents.Select((userId, idx) =>
                {
                    ...
                    var statusGroupCode = AgentStates[stateIdx];      // AVAILABLE, ONPHONE (ALL CAPS)
                    var agentState = AgentStateGroups[stateIdx];      // Available, On Phone (mixed case)
                    ...
                    {
                        // === Identity metrics (DisplayName function) ===
                        ["USERID"] = userId,
                        ...
                        // === Duration metrics (TotalStatusDuration function) ===
                        ...
                        // === Count metrics (InteractionsCount function) ===
