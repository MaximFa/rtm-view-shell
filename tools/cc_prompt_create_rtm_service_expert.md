# CC Task — create the rtm-service-expert project skill (coordinator-authored, verbatim)

> Coordinator-drafted (coordinator-0606), operator-approved. Creates a NEW project skill capturing the
> RTM Service engine + stored-routine knowledge (CLAUDE.md §33-34 + the 2026-06-06 bug-fix session).
> Content is AUTHORED BELOW — write it VERBATIM. .claude/ is gitignored -> `git add -f`. docs: commit.
...
## Sync (§42) — slug devops-0606. Claim: .claude/skills/rtm-service-expert/rtm-service-expert.md (NEW dir+file; free).
S1 barrier check (content-based). S2: coord_check_claims on the path. S3 phantom-aware lock. §40 skills read.
Writes Python+fsync (§0.3). NO push (§37).

## TASK — create the file VERBATIM
Make the directory and write the file below EXACTLY (Python+fsync; do not paraphrase):
`.claude/skills/rtm-service-expert/rtm-service-expert.md`

<<<<<<<<<< BEGIN FILE CONTENT (verbatim) >>>>>>>>>>
---
...
## Verify
```bash
head -3 .claude/skills/rtm-service-expert/rtm-service-expert.md
grep -c "RTM-SEC-002\|NGC_UserAgentgroup\|PROCEDURE\|fn_daytrendagentstatus" .claude/skills/rtm-service-expert/rtm-service-expert.md  # several
tail -2 .claude/skills/rtm-service-expert/rtm-service-expert.md
```

## Optional (domain owner): if you (daytrend/devops) spot an inaccuracy in the engine/SP details, fix it
## in the same commit — you have the deepest RTM context. Keep it factual, no invented APIs.

## Commit (docs module; .claude/ gitignored -> git add -f; §39.3)
```bash
bash tools/pre-commit-check.sh
git add -f .claude/skills/rtm-service-expert/rtm-service-expert.md
git commit -m "docs: add rtm-service-expert skill (RTM engine + stored-routine reference)"
bash tools/cc_post_commit.sh devops-0606 $(git log -1 --format=%h)
```
§0.6 verify + PD-007 re-sync. NO push — rides the next barrier. Also add a §30.3 row to CLAUDE.md
referencing the new skill IF convenient (optional; coordinator can do it).

## Peer review: coordinator-drafted; operator issues directly.
