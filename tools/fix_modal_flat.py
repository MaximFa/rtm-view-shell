#!/usr/bin/env python3
"""Fix DayTrend modal — revert to flat structure (no tabs)."""
import os
import re
...
def fix_razor():
    ...
    # 1. Remove _viewTab field
    text = text.replace(
        '    private string _viewTab = "general";  // "general" | "queue" | "agent"\n',
        ''
    )

    # 2. Remove _viewTab = "general"; from OpenLocalConfigAsync
    text = text.replace(
        '        _viewTab = "general";\n',
        ''
    )

    # 3. Replace tabbed modal body with flat version
    old_modal_body = '''<div class="modal-body p-0">
                        <!-- Tab nav -->
    ...
    new_modal_body = '''<div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
                        <!-- BU selector -->
                        <div class="mb-3">
                            <label class="form-label fw-semibold">@L["Widgets_BusinessUnit"]</label>
                            <select class="form-select form-select-sm" @bind="_localBuId">
